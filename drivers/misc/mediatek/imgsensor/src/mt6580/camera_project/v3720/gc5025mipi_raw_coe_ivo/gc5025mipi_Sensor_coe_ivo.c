...
#include "gc5025mipi_Sensor_coe_ivo.h"

#define GC5025_DEFAULT_DUMMY_PIXEL_NUMS   0x25c /* HB */
#define GC5025_DEFAULT_DUMMY_LINE_NUMS    0x08 /* VB */
static kal_uint32 Basic_Dummy_Line = GC5025_DEFAULT_DUMMY_LINE_NUMS;
/****************************Modify Following Strings for Debug****************************/
#define PFX "GC5025_COE_IVO_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC5025_COE_IVO_MIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define RG_TYPICAL    		0x0539
#define BG_TYPICAL			0x0545
...
typedef struct otp_gc5025_coe_ivo
{
...
static void gc5025_gcore_update_wb(void)
{
	...
	LOG_INF("GC5025_Define_AWB:rg_typical = 0x%x , bg_typical = 0x%x\n",rg_typical,bg_typical);//ackman update WB typical
	}
...
{
  kal_uint8 flag_doublereset,flag_GC5025_cxtA = 0;
	gc5025_gcore_initial_otp();
	gc5025_gcore_enable_otp(otp_open);

	gc5025_select_page_otp(otp_page1);
	flag_doublereset = gc5025_read_otp(0x26);
    flag_GC5025_cxtA = gc5025_read_otp(0x27);
	gc5025_select_page_otp(otp_page0);
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x3f, 0x91);	
    LOG_INF("Enable 5025 set dummy preview!\n");
...
static void capture_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x3f, 0x91);
	LOG_INF("ackman Enable 5025 set dummy capture!\n");
}
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x3f, 0x91);
	LOG_INF("ackman Enable 5025 set dummy video!\n");
}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
                        *sensor_id = *sensor_id +9;
			if (*sensor_id == imgsensor_info.sensor_id) {
				gc5025_gcore_identify_otp();
				
				LOG_INF("mtkfb_lcm_name = %s\n", mtkfb_lcm_name);
				if(gc5025_otp_info.module_id == 0x13 && !strcmp(mtkfb_lcm_name,"ili9881c_hd720_dsi_vdo_coe_ivo")){
...
UINT32 GC5025_COE_IVO_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
