...
static int bmi160_step_c_get_data(u64 *value, int *status)
{
	int err = 0;
	uint16_t step_counter_val = 0;

	err = bmi160_read_step_count(&step_counter_val);
	if (err) {
...
static int bmi160_stc_get_data_significant(u64 *value, int *status)
{
...
static int bmi160_stc_get_data_step_d(u64 *value, int *status)
{
