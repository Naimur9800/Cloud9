...
#if defined(CONFIG_PROJECT_V3760)
...
extern int AudDrv_GPIO_EXTAMP2_Select(int bEnable);
/* VOW using */
typedef enum {
	...
	if (enable) {
		...
		if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL] == false
			...
			if (mIsExtSPKUse) {
				...
				Ana_Set_Reg(AUDTOP_CON4, 0x007C, 0xffff);//LINE<DHACN-1189><20170118><changed 0x005c->0x007c for 2 line open .info look [FAQ14293]>wangyanhui
			} else {
...
static inline void AudDrv_GPIO_ExtAmps_Select(bool enable) // Jiangde++
{
    // printk(KERN_ERR"HJDDbgPA, AudDrv_GPIO_ExtAmps_Select(%d) \n", enable);
    AudDrv_GPIO_EXTAMP_Select(enable);
    // AudDrv_GPIO_EXTAMP2_Select(enable);
    udelay(GAP);
}
...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	...
	if (enable) {
...
#else
	#ifdef CONFIG_PROJECT_P4200
		AudDrv_GPIO_EXTAMP_Select(false);
		udelay(GAP);
		AudDrv_GPIO_EXTAMP_Select(true);
	#elif defined(CONFIG_PROJECT_V3730)
		AudDrv_GPIO_EXTAMP_Select(false);
		udelay(GAP);
    		AudDrv_GPIO_EXTAMP_Select(true);
    		udelay(GAP);
    		...
    		udelay(GAP);
    		AudDrv_GPIO_EXTAMP_Select(true);
        #elif defined(CONFIG_PROJECT_V2610)
		AudDrv_GPIO_EXTAMP_Select(false);
		udelay(GAP);
    		AudDrv_GPIO_EXTAMP_Select(true);
    		udelay(GAP);
    		...
    		AudDrv_GPIO_EXTAMP_Select(true);
                ...
                if(isHandFree && (FACTORY_BOOT != get_boot_mode())) {
                printk("Now is handfree mode");
    		//AudDrv_GPIO_EXTAMP_Select(false);
    		AudDrv_GPIO_EXTAMP2_Select(false);
    		udelay(GAP);
    		//AudDrv_GPIO_EXTAMP_Select(true);
	...
	#elif defined(CONFIG_PROJECT_V3720)
    		AudDrv_GPIO_EXTAMP_Select(false);
    		AudDrv_GPIO_EXTAMP2_Select(false);
    		udelay(GAP);
    		AudDrv_GPIO_EXTAMP_Select(true);
    		...
    		AudDrv_GPIO_EXTAMP_Select(true);
    		AudDrv_GPIO_EXTAMP2_Select(true);
#elif defined(CONFIG_PROJECT_V3740) // Jiangde++
        printk(KERN_ERR"HJDDbgPA, before cmd 01 01 01 \n");
        AudDrv_GPIO_ExtAmps_Select(0);
        AudDrv_GPIO_ExtAmps_Select(1);
        AudDrv_GPIO_ExtAmps_Select(0);
        AudDrv_GPIO_ExtAmps_Select(1);
        AudDrv_GPIO_ExtAmps_Select(0);
        AudDrv_GPIO_ExtAmps_Select(1);
        // printk(KERN_ERR"HJDDbgPA, after cmd \n");
#else
	
        #ifndef CONFIG_CUSTOM_KERNEL_ALSPS_SUB
    		AudDrv_GPIO_EXTAMP_Select(true);
    		udelay(GAP);
    		AudDrv_GPIO_EXTAMP_Select(false);
    		udelay(GAP);
        ...
        #else
    	    if(GsensorState == 2){
    		printk("Ext amp switch to L channel!");
    		AudDrv_GPIO_EXTAMP_Select(true);
    		//AudDrv_GPIO_EXTAMP2_Select(true);
    		udelay(GAP);
    		AudDrv_GPIO_EXTAMP_Select(false);
    		...
    		//AudDrv_GPIO_EXTAMP_Select(true);
    		//AudDrv_GPIO_EXTAMP2_Select(true);
    	    } else if(GsensorState == 1) {
    	    printk("Ext amp switch to R channel!");
    		//AudDrv_GPIO_EXTAMP_Select(true);
    		AudDrv_GPIO_EXTAMP2_Select(true);
    		udelay(GAP);
    		//AudDrv_GPIO_EXTAMP_Select(false);
...
//@chunshneg.xu program for STD_LRCH_EXT_SPEAKER start
static void Ext_Speaker_Amp_L_Change(bool enable)
{
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	if (enable) {
		pr_debug("Ext_Speaker_Amp_L_Change ON+\n");
		AudDrv_GPIO_EXTAMP2_Select(false);
		usleep_range(1*1000, 20*1000);
    		AudDrv_GPIO_EXTAMP2_Select(false);
    		udelay(GAP);
		...
		pr_debug("Ext_Speaker_Amp_L_Change OFF+\n");
		AudDrv_GPIO_EXTAMP2_Select(false);
		udelay(500);
		pr_debug("Ext_Speaker_Amp_L_Change OFF-\n");
	}
...
static void Ext_Speaker_Amp_R_Change(bool enable)
{
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	if (enable) {
		pr_debug("Ext_Speaker_Amp_R_Change ON+\n");
		AudDrv_GPIO_EXTAMP_Select(false);
		usleep_range(1*1000, 20*1000);
    		AudDrv_GPIO_EXTAMP_Select(false);
    		udelay(GAP);
		...
		pr_debug("Ext_Speaker_Amp_R_Change OFF+\n");
		AudDrv_GPIO_EXTAMP_Select(false);
		udelay(500);
		pr_debug("Ext_Speaker_Amp_R_Change OFF-\n");
	}
}
//@chunshneg.xu program for STD_LRCH_EXT_SPEAKER end
#else
#ifndef CONFIG_MTK_SPEAKER
#ifdef AW8736_MODE_CTRL
...
//@chunshneg.xu program for STD_LRCH_EXT_SPEAKER start
static int Ext_Speaker_Amp_L_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_warn("%s()\n", __func__);
	ucontrol->value.integer.value[0] =
	    mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP];
...
static int Ext_Speaker_Amp_L_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{

	pr_warn("%s() gain = %ld\n ", __func__, ucontrol->value.integer.value[0]);
	if (ucontrol->value.integer.value[0]) {
		Ext_Speaker_Amp_L_Change(true);
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP] =
		    ucontrol->value.integer.value[0];
	} else {
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP] =
		    ucontrol->value.integer.value[0];
		Ext_Speaker_Amp_L_Change(false);
	}
	return 0;
}

static int Ext_Speaker_Amp_R_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_warn("%s()\n", __func__);
	ucontrol->value.integer.value[0] =
	    mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP];
...
static int Ext_Speaker_Amp_R_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{

	pr_warn("%s() gain = %ld\n ", __func__, ucontrol->value.integer.value[0]);
	if (ucontrol->value.integer.value[0]) {
		Ext_Speaker_Amp_R_Change(true);
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP] =
		    ucontrol->value.integer.value[0];
	} else {
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP] =
		    ucontrol->value.integer.value[0];
		Ext_Speaker_Amp_R_Change(false);
	}
	return 0;
}
//@chunshneg.xu program for STD_LRCH_EXT_SPEAKER end
...
static const struct snd_kcontrol_new mt6350_snd_controls[] = {
	...
	SOC_ENUM_EXT("Ext_Speaker_Use", Audio_DL_Enum[13], Ext_Speaker_Use_Get,
		     Ext_Speaker_Use_Set),
        //@chunshneg.xu program for STD_LRCH_EXT_SPEAKER start
	SOC_ENUM_EXT("Ext_Speaker_Amp_L_Switch", Audio_DL_Enum[14], Ext_Speaker_Amp_L_Get,
		     Ext_Speaker_Amp_L_Set),
	SOC_ENUM_EXT("Ext_Speaker_Amp_R_Switch", Audio_DL_Enum[15], Ext_Speaker_Amp_R_Get,
		     Ext_Speaker_Amp_R_Set),
        //@chunshneg.xu program for STD_LRCH_EXT_SPEAKER end
};
