...
static int fts_switch_to_update(tinno_ts_data *ts)
{
	...
	{
//#if 0
	CTP_DBG("yolo....fts_switch_to_update zx \n");
...
//LINE <ft6x06> <DATE20140910> <modify for ft6336> yolo
 int get_device_ic_type(void){
	int ret = 0;
	uint8_t ic_type =0;
	
	ret = fts_read_reg(0xA3, &ic_type);
	CTP_DBG("yolo.......get_device_ic_type, ic_type=%x\n", ic_type);
	if (ret < 0){
		CTP_DBG("get_device_ic_type error, ret=%d\n", ret);
		return -1;
	}
	if(0x06==ic_type){
		ic_type_flag = 0;//ft6x06
	}else if((0x36==ic_type) ||(0x64==ic_type))
	{
		ic_type_flag = 1;//ft6336
	}
	CTP_DBG("yolo.......get_device_ic_type, ic_type_flag=%d\n", ic_type_flag);
	return 0;	
}
	...
	//LINE <ft6x06> <DATE20140910> <modify for ft6336> yolo
 	...
 	do{
		...
		ft6x06_get_tp_id(ts, &tp_id);
		 CTP_DBG("yolo.....entry ft6x06_get_tp_id tp_id=%x \n",tp_id);
		 CTP_DBG("yolo.....entry ft6x06_get_tp_id tp_id=%x \n",tp_id);
		 if(FTS_CTP_FIRWARE_ID != tp_id ){
			ic_type_flag =1;
		 }
		if(( FTS_CTP_FIRWARE_ID == tp_id )||(0x7918 == tp_id)||(0x791C == tp_id)){
		//rc = -EINVAL;
		//goto err;
    	    CTP_DBG("check id OK \n");
    	    break;
...
int ft6x06_get_vendor_version(tinno_ts_data *ts, uint8_t *pfw_vendor, uint8_t *pfw_version)
{
	...
	*pfw_version = ft6x06_get_fw_version(ts);
	...
	get_device_ic_type();//LINE <ft6x06> <DATE20140910> <modify for ft6336> yolo
