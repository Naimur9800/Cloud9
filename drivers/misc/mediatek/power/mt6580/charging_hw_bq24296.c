...
static unsigned int charging_hw_init(void *data)
{
	...
	bq24296_set_vindpm(0x7);	/* VIN DPM check 4.68V(0xA) ->4.44V(0x7)*/
	...
	bq24296_set_batlowv(0x1);	/* BATLOWV 3.0V */
        ...
        #if (defined(CONFIG_PROJECT_V3800)||defined(CONFIG_PROJECT_V3610)||defined(CONFIG_PROJECT_V3760))
        bq24296_set_vrechg(0x0);
        #else
	bq24296_set_vrechg(0x1);	/* VRECHG 0.3V (4.108V) */	
        #endif	
...
static unsigned int charging_set_cv_voltage(void *data)
{
	...
	//battery_log(BAT_LOG_CRTI,"[BQ24296] cv_value=%d,set_cv_voltage=%d,register_value=%d \n",cv_value,set_cv_voltage,register_value);
