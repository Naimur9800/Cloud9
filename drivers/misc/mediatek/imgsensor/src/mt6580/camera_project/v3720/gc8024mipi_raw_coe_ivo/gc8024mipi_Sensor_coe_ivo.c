...
#include "gc8024mipi_Sensor_coe_ivo.h"

#define GC8024_DEFAULT_DUMMY_PIXEL_NUMS   0x21c // HB
#define GC8024_DEFAULT_DUMMY_LINE_NUMS    0x10 //VB
static kal_uint32 Basic_Dummy_Line = GC8024_DEFAULT_DUMMY_LINE_NUMS;//travis
/****************************Modify Following Strings for Debug****************************/
#define PFX "GC8024_COE_IVO_camera_sensor"
#define LOG_1 LOG_INF("GC8024_COE_IVO\n")
/****************************   Modify end    *******************************************/
...
static kal_bool GC8024DuringTestPattern = KAL_FALSE;
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC8024_COE_IVO_MIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define RG_TYPICAL   				0x0092	//modified when write WB !!!
#define BG_TYPICAL				0x0092	//modified when write WB!!!
#define WINDOW_WIDTH  			0x0cd0 //3280 max effective pixels
#define WINDOW_HEIGHT 			0x099c //2460
#define DD_PARAM_QTY 			108
#define LSC_WIDTH 				6
#define LSC_HEIGHT 				6
u16 otp_gc8024_coe_ivocxt_af_data[2];

typedef struct otp_gc8024_coe_ivo{
	...
	kal_uint16 af_flag;// 0:Empty 1:Success 2:Invalid
...
{
	kal_uint8 flag1,flag2,flag_af;
	kal_uint8 index,i,j,cnt=0;
	kal_uint8 info_start_add,wb_start_add,af_start_add;
...
static int16_t para_medium[108] = {
	...
	flag_af = gc8024_read_otp(0x7c);	
	LOG_INF("GC8024_OTP flag1 = 0x%x\n",flag1);
	LOG_INF("GC8024_OTP flag_af = 0x%x\n",flag_af);
...
/*AF-170714BRYAN*/
	for(index=0;index<2;index++)
	{
		switch((flag_af>>(6 - 2 * index))&0x03)
		{
		case 0x00:
			LOG_INF("GC8024_OTP_AF group%d is Empty !!\n", index + 1);
			break;
		case 0x01:
			LOG_INF("GC8024_OTP_AF group%d is Valid !!\n", index + 1);
			if(index==0)
			{
				gc8024_otp_select_page(otp_page0);
				af_start_add = 0x7d;
				LOG_INF("GC8024_OTP_AF page0 !!\n");
			}
			else
			{
				gc8024_otp_select_page(otp_page1);
				af_start_add = 0x7a;
				LOG_INF("GC8024_OTP_AF page1 !!\n");
				
			}
			gc8024_otp_info.vcm_start = ((gc8024_read_otp(af_start_add)<<2)&0x03fc)+((gc8024_read_otp(af_start_add + 2)>>6)&0x03);  //远焦
			gc8024_otp_info.vcm_end = ((gc8024_read_otp(af_start_add + 1)<<2)&0x03fc)+((gc8024_read_otp(af_start_add + 2)>>4)&0x03); //进焦
			gc8024_otp_info.vcm_dir = (gc8024_read_otp(af_start_add + 2)>>2)&0x03;	//01upward, 10horizon,11 downward	
			LOG_INF("GC8024_OTP_AF group%d is valid !!\n", index + 1);
			otp_gc8024_coe_ivocxt_af_data[0] = gc8024_otp_info.vcm_start;
			otp_gc8024_coe_ivocxt_af_data[1] = gc8024_otp_info.vcm_end;
			LOG_INF("GC8024_OTP_AF vcm_start=0x%x\n",gc8024_otp_info.vcm_start);
			LOG_INF("GC8024_OTP_AF vcm_end=0x%x\n",gc8024_otp_info.vcm_end);
			LOG_INF("GC8024_OTP_AF vcm_dir=0x%x\n",gc8024_otp_info.vcm_dir);
			break;
		case 0x02:
		case 0x03:	
			LOG_INF("GC8024_OTP_AF group%d is Invalid !!\n", index + 1);
			break;
		default :
			break;
		}		
...
static void gc8024_gcore_update_awb(void)
{
	...
	if(0x01==(gc8024_otp_info.wb_flag&0x01))
	{
		...
		LOG_INF("GC8024 OTP RG_TYPICAL = 0x%x , BG_TYPICAL = 0x%x \n",RG_TYPICAL,BG_TYPICAL);
...
static void gc8024_gcore_update_lsc(void)
{
	...
	if(0x01 == gc8024_otp_info.lsc_flag)
	{
		...
		for(i=0; i<6; i++)
		{
			...
			write_cmos_sensor(0xa0,0x60);//[0]LSC Enable	//0x61
			write_cmos_sensor(0xfe,0x00);
	}
}
...
static void set_dummy(void)
{
 	kal_uint32 hb = 0;
	kal_uint32 vb = 0;
	LOG_INF("0810 dummyline = %d, Basic_Dummy_Line = %d \n", imgsensor.dummy_line, Basic_Dummy_Line);

	hb = imgsensor.dummy_pixel + GC8024_DEFAULT_DUMMY_PIXEL_NUMS;
	vb = imgsensor.dummy_line*4 + Basic_Dummy_Line;
...
//	LOG_INF("---0103---shutter =%d, framelength =%d, Basic_Dummy_Line = %d !!!!!!!!\n", shutter,imgsensor.frame_length,Basic_Dummy_Line);
		...
		do {
			...
			*sensor_id = *sensor_id+2;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				LOG_INF("mtkfb_lcm_name = %s\n", mtkfb_lcm_name);
				
				if(gc8024_otp_info.module_info1== 0x13 && !strcmp(mtkfb_lcm_name,"ili9881c_hd720_dsi_vdo_coe_ivo")){
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    LOG_INF("---0810--scenario_id = %d, framerate = %d\n", scenario_id, framerate);
    ...
    switch (scenario_id) {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            ...
            imgsensor.min_frame_length = imgsensor.frame_length;
            spin_unlock(&imgsensor_drv_lock);			
			Basic_Dummy_Line = imgsensor.min_frame_length*4 - 2476;//travis
            //set_dummy();
            set_max_framerate(framerate,0);
			LOG_INF("0810 Basic_Dummy_Line = %d \n", Basic_Dummy_Line);
            break;
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
            if(framerate == 0)
                return ERROR_NONE;
...
UINT32 GC8024_COE_IVO_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
