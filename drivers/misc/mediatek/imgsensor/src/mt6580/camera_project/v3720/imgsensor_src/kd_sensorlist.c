...
char zsd_str[] = "zsdoff"; //liaoshuang add for mt6580 13M zsd off
/* Camera Infos */
			...
			if (DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]) {
				...
				if (sensorID == 0) {	/* not implement this feature ID */
					...
					snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
						 ...
						 g_invokeSocketIdx[i], g_invokeSensorNameStr[i]);
					//++liaoshuang add for mt6580 13M zsd off
					 if ((0 == strcmp(SENSOR_DRVNAME_OV13850_MIPI_RAW, g_invokeSensorNameStr[i])) || 
							(0 == strcmp(SENSOR_DRVNAME_IMX135_OFILM_MIPI_RAW, g_invokeSensorNameStr[i])) || 
								(0 == strcmp(SENSOR_DRVNAME_IMX135_SUNWIN_MIPI_RAW, g_invokeSensorNameStr[i])))
					 {
						strncat(mtk_ccm_name,zsd_str,sizeof(zsd_str));						
						PK_INF("ls.zsdstr[] = %s\n,g_invokeSensorNameStr = %s\n",mtk_ccm_name,g_invokeSensorNameStr[i]);
					 }
					 //--liaoshuang add for mt6580 13M zsd off
