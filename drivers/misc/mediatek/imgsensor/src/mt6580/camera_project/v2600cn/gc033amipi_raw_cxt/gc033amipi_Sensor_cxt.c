...
#include "gc033amipi_Sensor_cxt.h"

#define GC033a_DEFAULT_DUMMY_PIXEL_NUMS   0x37b // HB
#define GC033a_DEFAULT_DUMMY_LINE_NUMS    0x06 //VB
static kal_uint32 Basic_Dummy_Line = GC033a_DEFAULT_DUMMY_LINE_NUMS;//travis


/****************************Modify Following Strings for Debug****************************/
#define PFX "GC033A_CXT_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC033A_CXTMIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
    ...
    .margin = 2,            //sensor framelength & shutter margin 0 ackman
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
    ...
    set_dummy();
	LOG_INF("--ackman max frame_length = %d \n", frame_length);
}    /*    set_max_framerate  */
...
static void set_shutter(kal_uint16 shutter)
{
	...
	{     	
		...
		{		
	    	set_max_framerate(realtime_fps,0);
		} *///ackman delete to avoid screen-cut  
	#endif
	// Update Shutter
	...
	LOG_INF("--ackman set shutter realfps = %d \n", realtime_fps);
...
{	
	...
	{ 
		...
		LOG_INF("GC033A_CXTMIPI analogic gain 1x, GC033A_CXTMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC033A_CXTMIPI analogic gain 1.42x , GC033A_CXTMIPI add pregain = %d\n",temp);
	}
	...
	{ 
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC033A_CXTMIPI analogic gain 1.97x , GC033A_CXTMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC033A_CXTMIPI analogic gain 2.78x , GC033A_CXTMIPI add pregain = %d\n",temp);
	}
	else 
	{
		//analog gain
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC033A_CXTMIPI analogic gain 3.78x , GC033A_CXTMIPI add pregain = %d\n",temp);
	}
	return gain;
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("GC033A_CXT get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("GC033A_CXT get_imgsensor_id Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    sensor_info->SensorHorFOV = 43;
    sensor_info->SensorVerFOV = 37;
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    switch (scenario_id) {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            ...
            imgsensor.min_frame_length = imgsensor.frame_length;
            spin_unlock(&imgsensor_drv_lock);
			Basic_Dummy_Line = imgsensor.min_frame_length - 500;//travis
            //set_dummy();
...
{
    LOG_INF("ackman default scenario_id = %d\n", scenario_id);
...
UINT32 GC033A_CXTMIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC033A_CXTMIPI_RAW_SensorInit    */
