...
//#define IMX135_ofilm_OTP(fmt,arg...)  pr_err(PFX "[%s] " format, __FUNCTION__, ##args)
...
**************************************************************************************************/
 BYTE ofilm_get_otp_module_id(void)
{
    BYTE module_id = 0;
	BYTE id_zone=0;
if(get_otp_flag(0x02) == 0x55) //page 1 and page 2 for awb
		id_zone = 0x02;
	else
		id_zone = 0x01;	
    ...
    module_id = read_cmos_sensor(0x3B08);
...
static BYTE get_otp_lens_id(BYTE zone)
{
    ...
    lens_id = read_cmos_sensor(0x3B0a);
...
static bool otp_lenc_update(BYTE zone)
{
	...
	memcpy(lsc_data,lsc_data_mask+1,sizeof(lsc_data));
#ifdef imx135_otp_ofilm_debug
	for ( k=0;k<378;k++){
...
/*    temph = read_cmos_sensor(0x3B18);
	...
	current_r = (read_cmos_sensor(0x3B0a) << 8) | read_cmos_sensor(0x3B0b);
	current_b = (read_cmos_sensor(0x3B0c) << 8) | read_cmos_sensor(0x3B0d);
	current_gr = current_gb = (read_cmos_sensor(0x3B0e) << 8) | read_cmos_sensor(0x3B0f);
...
void otp_clear_flag_ofilm(void){
...
bool otp_update_ofilm(BYTE update_sensor_otp_awb, BYTE update_sensor_otp_lsc)
{
...
//awb data
	if(get_otp_flag(0x02) == 0x55) //page 1 and page 2 for awb
		awb_zone = 0x02;
	else if(get_otp_flag(0x01) == 0x55)
		awb_zone = 0x01;	
	LOG_INF("awb_zone=%d\n",awb_zone);
	if(0 != update_sensor_otp_awb && _otp_awb_set == 0) {
	LOG_INF("update_sensor_otp_awb == 1");
...
//	ofilm_Module_ID = get_otp_module_id(awb_zone);
//	LOG_INF("ofilm_Module_ID = %d\n",ofilm_Module_ID);
//lsc data
	if(get_otp_flag(0x09) == 0x40)//page 3-8 and page 9-14 for lsc
		lsc_zone = 0x09;
	else if(get_otp_flag(0x03) == 0x40)
		lsc_zone = 0x03;
	LOG_INF("lsc_zone=%d\n",lsc_zone);
	 if(0 != update_sensor_otp_lsc && _otp_lsc_set == 0)
    {
...
//af data 
    ...
    {
    	if(read_cmos_sensor(0x3B04) == 0x55){
		ofilm_af_mac = (read_cmos_sensor(0x3b05) << 8) | (read_cmos_sensor(0x3b06));
		ofilm_af_inf = (read_cmos_sensor(0x3b07) << 8) | (read_cmos_sensor(0x3b08));
    		}
	else if(read_cmos_sensor(0x3B18) == 0x55){
		ofilm_af_mac = (read_cmos_sensor(0x3b19<< 8) | (read_cmos_sensor(0x3b1a)));	
		ofilm_af_inf = (read_cmos_sensor(0x3b1b<< 8) | (read_cmos_sensor(0x3b1c)));
    }
	else{
	ofilm_af_mac = 0;
	ofilm_af_inf = 0;   
   }
	LOG_INF("ofilm_af_mac = %d,ofilm_af_inf = %d\n",ofilm_af_mac,ofilm_af_inf);
}
...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "IMX135_OFILM_CAM_CAL_DRV"
...
u8 imx135_ofilm_id_data[]= {0x10,0xee,0x00,0x40,0x88};
static int selective_read_region(u32 offset, BYTE* data,u16 i2c_id,u32 size)
{    
    printk("[IMX135_ofilm_CAM_CAL] selective_read_region offset =%d size %d data read = %d\n", offset,size, *data);
if(size ==2 ){
	if(offset == 7){
		memcpy((void *)data,(void *)&ofilm_af_inf,size);
	}
	
	else if(offset == 9){
		 
		memcpy((void *)data,(void *)&ofilm_af_mac,size);

	}
}
if(size == 4){
		memcpy((void *)data,(void *)&imx135_ofilm_id_data[1],size);
...
#endif
{
    ...
    else
    {
        ...
        if(NULL == pBuff)
        {
            LOG_INF("[IMX135_ofilm_CAM_CAL] ioctl allocate mem failed\n");
            return -ENOMEM;
        }
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                LOG_INF("[IMX135_ofilm_CAM_CAL] ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
...
//fix warning MSG     LOG_INF("[IMX135_ofilm_CAM_CAL] init Working buffer address 0x%x  command is 0x%08x\n", pWorkingBuff, a_u4Command);
    ...
    LOG_INF("[LOG_INF] CAM_CALIOC_S_WRITE \n");        
        ...
        case CAM_CALIOC_G_READ:
            LOG_INF("[IMX135_ofilm_CAM_CAL] Read CMD \n");
#ifdef CAM_CALGETDLT_DEBUG            
            do_gettimeofday(&ktv1);
#endif 
            LOG_INF("[IMX135_ofilm_CAM_CAL] offset %d \n", ptempbuf->u4Offset);
            LOG_INF("[IMX135_ofilm_CAM_CAL] length %d \n", ptempbuf->u4Length);
          // LOG_INF("[IMX135_ofilm_CAM_CAL] Before read Working buffer address 0x%x \n", pWorkingBuff);
...
if(ptempbuf->u4Length == 2){
	   ...
	   i4RetValue = selective_read_region(ptempbuf->u4Offset, pWorkingBuff, 0x20, ptempbuf->u4Length);
          ...
          // LOG_INF("[IMX135_ofilm_CAM_CAL] After read Working buffer address 0x%x \n", pWorkingBuff);
            ...
            break;
        default :
      	     LOG_INF("[IMX135_ofilm_CAM_CAL] No CMD \n");
            i4RetValue = -EPERM;
        break;
    }
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        LOG_INF("[IMX135_ofilm_CAM_CAL] to user length %d \n", ptempbuf->u4Length);
		...
		if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pWorkingBuff , ptempbuf->u4Length))
        {
            kfree(pBuff);
            kfree(pWorkingBuff);
            LOG_INF("[IMX135_ofilm_CAM_CAL] ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
//#else
    ...
    //{
    //    LOG_INF("[IMX135_ofilm_CAM_CAL] Fail to enable analog gain\n");
    //    return -EIO;
    //}
//#endif	
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    {
        LOG_INF("[IMX135_ofilm_CAM_CAL] Allocate device no failed\n");
    ...
    {
        LOG_INF("[IMX135_ofilm_CAM_CAL] Register device no failed\n");
    ...
    if(NULL == g_pCAM_CAL_CharDrv)
    {
        ...
        LOG_INF("[IMX135_ofilm_CAM_CAL] Allocate mem for kobject failed\n");
    ...
    {
        LOG_INF("[IMX135_ofilm_CAM_CAL] Attatch file operation failed\n");
    ...
    CAM_CAL_class = class_create(THIS_MODULE, "IMX135_ofilm_CAM_CALdrv");
...
static int __init CAM_CAL_i2C_init(void)
{
   ...
   // i2c_register_board_info(CAM_CAL_I2C_BUSNUM, &kd_cam_cal_dev, 1);
	 ...
	 int i4RetValue = 0;
    LOG_INF("[IMX135_ofilm_CAM_CAL]\n");
   //Register char driver
	i4RetValue = RegisterCAM_CALCharDrv();
    if(i4RetValue){
 	   LOG_INF(" [IMX135_ofilm_CAM_CAL] register char device failed!\n");
	   return i4RetValue;
	}
	LOG_INF(" [IMX135_ofilm_CAM_CAL] Attached!! \n");
...
LOG_INF(" IMX135_ofilm_CAM_CAL  Attached Pass !! \n");
    return 0;
}
