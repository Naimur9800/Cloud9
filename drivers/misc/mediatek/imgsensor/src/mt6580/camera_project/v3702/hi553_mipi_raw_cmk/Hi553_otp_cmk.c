...
signed char check_MID_hi553(bool bOnlyCheck);
...
bool otp_update_wb_hi553(unsigned short golden_rg, unsigned short golden_bg) 
{
	...
	if(DEBUG_PRINT)
	printk("[Hi553 ]enter otp_update_wb_hi553 r_gain=0x%x b_gain = 0x%x  \n",r_gain,b_gain);
...
signed char hi553_get_chip_information(void)
{
	...
	printk("[Hi553 ]geroge enter hi553_get_chip_information =0x%x \n",otp_flag);
	if(otp_flag == OTP_MID)
		return 1;
	else 
		return 0;
...
signed char hi553_get_cali_wb(void)
{
	...
	switch(otp_flag)
	{
		...
		for(i = 0; i < HI553_CALI_WB_DATA_LENGTH; i++)
		{
			...
			if(DEBUG_PRINT)
			printk("[Hi553 ] hi553_get_cali_wb wb_data[%d] == 0x%x ,\n",i,wb_data[i]);
			checksum += wb_data[i];
		}
		otp_flag = hi553_read_cmos_sensor_for_otp(0x108);
		...
		case 0x13:		
			...
			for(i = 0; i < HI553_CALI_WB_DATA_LENGTH; i++)
			{
				...
				printk("[Hi553 ] hi553_get_cali_wb flag= 0x13 wb_data[%d] == 0x%x ,\n",i,wb_data[i]);
				checksum += wb_data[i];
			}
			otp_flag = hi553_read_cmos_sensor_for_otp(0x108);
			checksum = checksum%0xff + 1;
		...
		case 0x37:		
			...
			for(i = 0; i < HI553_CALI_WB_DATA_LENGTH; i++)
			{
				wb_data[i] = hi553_read_cmos_sensor_for_otp(0x108);
				if(DEBUG_PRINT)
				printk("[Hi553 ] hi553_get_cali_wb flag= 0x37 wb_data[%d] == 0x%x ,\n",i,wb_data[i]);
				checksum += wb_data[i];
			}
			otp_flag = hi553_read_cmos_sensor_for_otp(0x108);
			checksum = checksum%0xff + 1;
...
signed char check_MID_hi553(bool bOnlyCheck)
{
		...
		if(DEBUG_PRINT)
		printk("[Hi553 ]enter check_MID_hi553g =0x%x \n",bOnlyCheck);
