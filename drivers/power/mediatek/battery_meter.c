...
//liaoye@2016/11/04, start
struct timespec  g_sleep_total_time;
signed int g_reflash_timer = 150;
signed int g_sleep_oam_time = 0;
signed int oam_delta_time = 0;
signed int sleep_reflash_oam = 0;
//liaoye@2016/11/04, end
...
//liaoye@2016/10/25, fix bugid:DHACN-785, start
//parse the kernel cmdline, get the vbat and ibat
#ifdef GET_OCV_FROM_PL
#define BAT_VOL_STR  "tinno_vbat="
#define BAT_IVOL_STR  "tinno_ibat="
extern char *saved_command_line;
int get_voltage_from_pl_by_type(int type)
{
	char *start;
	char buf[10];
	int i, ret, data;
	...
	switch (type) {
		case 1: 
			start = strstr(saved_command_line, BAT_IVOL_STR);
			if (start == NULL)
				return 0;
  			start = start + strlen(BAT_IVOL_STR);
			break;
		case 2: 
			start = strstr(saved_command_line, BAT_VOL_STR);
			if (start == NULL)
				return 0;
  			start = start + strlen(BAT_VOL_STR);
			break;
		default:
			return 0;
	}
...
#endif
//liaoye@2016/10/25, fix bugid:DHACN-785, end
...
#ifdef GET_OCV_FROM_PL
static int g_ibat, g_vbat, g_rbat, g_current, g_voltage, g_soc;
#endif
...
//liaoye@2016/10/25, fix bugid:DHACN-785, start
//calcation the ocv, current = (Ibat - Vbat) / Rbat
//ocv = Vbat - current * Rbat
#ifdef GET_OCV_FROM_PL
	int ibat, vbat;
	int rbat;	//battery internal resistor
	int real_current;
	...
	ibat = get_voltage_from_pl_by_type(1);
	vbat = get_voltage_from_pl_by_type(2);
	if (ibat != 0 && vbat != 0) {
		real_current = (ibat - vbat) * 1000 / batt_meter_cust_data.cust_r_sense;

		rbat = fgauge_read_r_bat_by_v(vbat);
	
		//resistance:  battery connector resistance + battery internal resistance
		gFG_voltage = vbat - (real_current * (rbat + BATTERY_CONTECT_RESISTANCE)) / 1000;
		...
		g_vbat = vbat;
		g_rbat = rbat;
		g_current = real_current;
		g_voltage = gFG_voltage;
		g_soc = gFG_capacity_by_v ;
	} else {
...
#ifdef GET_OCV_FROM_PL
	printk("###paul ibat = %d, vbat = %d, %d, %d, %d, %d\n", 
		g_ibat, g_vbat, g_current, g_voltage, g_soc, g_rbat);
#endif
	oam_car_1 = (oam_i_1 * delta_time / 3600) + oam_car_1;	/* 0.1mAh */
	oam_car_2 = (oam_i_2 * delta_time / 3600) + oam_car_2;	/* 0.1mAh */

// TINNO BEGIN 
// Added by liaoye on Nov. 14, 2016 for fixed:
// according the temperature to select the bat capacity, start
	gFG_BATT_CAPACITY_aging =
		fgauge_get_Q_max(battery_meter_get_battery_temperature());
// TINNO END
	...
	if (d5_count >= d5_count_time) {
		if (gFG_Is_Charging == KAL_FALSE) {
			//liaoye@2016/11/04, Optimization oam_d_5
			if (oam_d_3 > oam_d_5) {
              if(!bat_spm_timeout)
                oam_d_5 = oam_d_5 + 1;
              else {
...
//liaoye@2016/11/04, update the battery parameters when out sleep, start
#if defined(SOC_BY_SW_FG)
...
//liaoye@2016/11/04, update the battery parameters when out sleep, end
...
//liaoye@2016/11/04, update the battery parameters when out sleep, start
	...
	battery_log(BAT_LOG_CRTI, "[battery_meter_resume] vol_bat1=%d, HW_OCV1=%d\n", vol_bat1 ,HW_OCV1);
	battery_log(BAT_LOG_CRTI, "[battery_meter_resume] D0=%d,UI_SOC_TEMP=%d  oam=%d %d %d %d\n", D0,100-D0, oam_d0, oam_d_2, oam_d_5, gFG_BATT_CAPACITY_aging);

	battery_log(BAT_LOG_CRTI,
		 "[battery_meter_resume]hw_ocv_after_sleep=(%d)v, g_hw_ocv_before_sleep=(%d)v \n",
		 hw_ocv_after_sleep, g_hw_ocv_before_sleep);

	//liaoye@2016/11/04, the total time is up to 60s to update 
	//the CAR
	if (_g_bat_sleep_total_time > 60) {
		if ((hw_ocv_after_sleep <= g_hw_ocv_before_sleep) && (BMT_status.UI_SOC>100-D0))
		{   
			//liaoye@2016/11/04, the total time is up to 1800s to update 
			//the D0 and reset CAR
			...
			battery_log(BAT_LOG_CRTI, "[battery_meter_resume] RS=%d, oam_delta_time=%ds, sleeptime=%ds, reflash=%d\n", D0 - oam_d0, oam_delta_time, _g_bat_sleep_total_time, sleep_reflash_oam);
			oam_d0 = D0;
