/* Copyright (C) MicroArray * MicroArray Fprint Driver Code for REE enviroment
 * madev.c
 * Date: 2016-12-18
 * Version: v4.0.04
 * Author: guq
...
static unsigned int screen_flag=0;
...
static DECLARE_WAIT_QUEUE_HEAD(screen_waitq);
...
//extern struct mt_chip_conf smt_conf;
...
static void mas_set_input(void) {
	...
	set_bit(FINGERPRINT_SWIPE_POWER, input->keybit); //单触
...
static long mas_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
    ...
    switch(cmd){
		...
		input_report_key(smas->input, FINGERPRINT_SWIPE_POWER, 1);
		input_sync(smas->input);
		input_report_key(smas->input, FINGERPRINT_SWIPE_POWER, 0);
		input_sync(smas->input);
		...
		case WAIT_SCREEN_STATUS_CHANGE:
			screen_flag = 0;
			ret = wait_event_freezable(screen_waitq, screen_flag != 0);
			mutex_lock(&ioctl_lock);
