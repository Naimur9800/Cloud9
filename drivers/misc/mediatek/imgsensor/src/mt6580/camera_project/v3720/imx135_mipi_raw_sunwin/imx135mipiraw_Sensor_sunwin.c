...
#define PFX "IMX135_SUNWIN_camera_sensor"
...
#define IMX135_OTP_Enable
static DEFINE_SPINLOCK(imgsensor_drv_lock);
 extern BYTE sunwin_get_otp_module_id(void);
#define MIPI_SETTLEDELAY_AUTO     0
#define MIPI_SETTLEDELAY_MANNUAL  1
#define IHDR_USED
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX135_SUNWIN_SENSOR_ID,
    ...
    .cap = {   // 15  fps  capture
        ...
        .mipi_data_lp2hs_settle_dc = 30,
        .max_framerate = 150, //mt6580 13M full max 15fps
    },
    .cap1 = {    // 24 fps  capture
        .pclk = 348000000,
        .linelength = 4572,
...
#else
extern bool otp_update_sunwin(BYTE update_sensor_otp_awb, BYTE update_sensor_otp_lsc);
extern void otp_clear_flag_sunwin(void);
#endif
...
/*
Add by chuansheng.fang@tinno.com
Main jobs:lsc otp off when ae gain > 10x;10x->230
0x0205 [7:0] ana_gain_global 0~240
ana_gain_golba	Gain(times)	Gain(dB)
	225				8.26	18.34
	230				9.85	19.87
	240				16.0	24.08
*/
	...
	if(reg_gain > 230)
	{
		if(0 != imgsensor.update_sensor_otp_lsc)
		{
			// disable sensor LSC
			temp1 = temp1 & 0xFE;	
			write_cmos_sensor(0x0700, temp1);
			imgsensor.update_sensor_otp_lsc = 0;
		}			
	} else {
		if(0 == imgsensor.update_sensor_otp_lsc)
		{
			...
			temp1 = temp1 | 0x01;	
			write_cmos_sensor(0x0700, temp1);
			imgsensor.update_sensor_otp_lsc = 1;
		}
}
#endif
    // SE  Gain
...
static void sensor_init(void)
{
    ...
    */
	...
	//#if IMX135_OTP_Enable
#ifdef IMX135_OTP_Enable
	otp_update_sunwin(1,1);
#endif

    imgsensor.update_sensor_otp_awb = 1; // Init to 0
    imgsensor.update_sensor_otp_lsc = 1; // Init to 0
...
#endif
static void IMX135MIPI_set_Video_IHDR(kal_uint16 IHDR_En)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {//(sunwin_get_otp_module_id()==0x06)
            		if(sunwin_get_otp_module_id()==0x06){
#ifdef CONFIG_MTK_CAM_CAL
...
static kal_uint32 close(void)
{
	...
	otp_clear_flag_sunwin();//clear otp flag
#endif
                          ...
                          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if (imgsensor.current_fps == 300) {
		LOG_INF("++++ls.test.mgsensor.current_fps == 300");
	...
	LOG_INF("++++ls.test.mgsensor.current_fps == 150");
    ...
    LOG_INF("++++ls.test.mgsensor.current_fps = %d\n",imgsensor.current_fps);
 ...
 //   if(imgsensor.current_fps == 150)  //liaoshuang add for 3730 13M max 15fps
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_SET_SENSOR_OTP_AWB_CMD:
            ...
            if(0 != imgsensor.update_sensor_otp_awb || 0 != imgsensor.update_sensor_otp_lsc) {
                otp_update_sunwin(imgsensor.update_sensor_otp_awb, imgsensor.update_sensor_otp_lsc);
            }
#endif
            break;
...
UINT32 IMX135_SUNWIN_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
