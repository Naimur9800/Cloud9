...
static int LM3643_write_reg(struct i2c_client *client, u8 reg, u8 val)
{
	...
	printk("LM3643_write_reg weiwei reg = %d\n",reg);
	printk("LM3643_write_reg  weiwei val = %d\n",val);
...
static int torchDuty[e_DutyNum]=    {35,71,106,127,0,0,0,0,0,0,0,0};
//50,100,150,179ma
static int flashDuty[e_DutyNum]=     {42,42,46,50,55,59,63,67,72,76,80,84};
...
{
	//	printk("LM3643_write_reg_ext reg = %d\n",reg);
	//	printk("LM3643_write_reg_ext val = %d\n",val);
...
int FL_Enable(void)
{
	...
	LM3643_write_reg(LM3643_i2c_client,LM3643_REG_TIMING, 0x1F); //0x1F  

	compatile_id = readReg(LM3643_REG_COMPATIB);

	PK_DBG(" FL_Enable compatile_id = %d\n",compatile_id);
	...
	}else 
	{
		...
		{
			PK_DBG(" FL_Enable line=%d\n",__LINE__);
			LM3643_write_reg(LM3643_i2c_client,LM3643_REG_TORCH_LED1, 0x6a);//150ma
			LM3643_write_reg(LM3643_i2c_client,LM3643_REG_ENABLE, temp|0x09);			 
		}
		else
		{
