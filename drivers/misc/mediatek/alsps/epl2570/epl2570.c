 ...
 * driver info
*******************************************************************************/
#define EPL_DEV_NAME   		    "EPL2570"
#define DRIVER_VERSION          "3.0.0_TINNO"
/******************************************************************************
...
#define ALS_DYN_INTT    1   // ALS Auto INTT  1
#define HS_ENABLE       0   // heart beat
#define PS_TINNO_DYN 	1

#define ALS_TINNO_DYN 	1

#define ANTI_BOUNCE_SET 	1


#define LUX_PER_COUNT	400  //ALS lux per count  700
/*ALS interrupt table*/
unsigned long als_lux_intr_level[] = {15, 39, 63, 316, 639, 4008, 5748, 10772, 14517, 65535};
unsigned long als_adc_intr_level[10] = {0};
#if (defined(CONFIG_PROJECT_V3800)||defined(CONFIG_PROJECT_V3802)||defined(CONFIG_PROJECT_V3610))
unsigned long		tinno_als_level[15]	= {26, 52, 96, 188, 1796, 4100, 6537, 9526, 12891, 16439, 20076, 24059, 65535, 65535, 65535};
#elif defined(CONFIG_PROJECT_V2600)
unsigned long		tinno_als_level[15]	= {13, 29, 80, 162, 1094, 2873, 6113, 8914, 13252, 17493, 22393, 27685, 65535, 65535, 65535};
#else
unsigned long		tinno_als_level[15]	= {30, 69, 150, 272, 2470, 5650, 9014, 13105, 17729, 22588, 27575, 33029, 65535, 65535, 65535};
#endif

unsigned long		tinno_als_value	[15]= {0 ,38 ,95 ,190 ,1250, 1500 ,1700, 1920 ,2900, 5745 ,8500 ,10000, 10243, 10243, 10243};
...
struct platform_device *epl2570_alspsPltFmDev;
...
#ifdef ALS_TINNO_DYN
static int als_dynamic_intt_intt[] = {EPL_ALS_INTT_128, EPL_ALS_INTT_128};
static int als_dynamic_intt_value[] = {128, 128}; //match als_dynamic_intt_intt table
...
#if !(defined(CONFIG_PROJECT_V3800)||defined(CONFIG_PROJECT_V3610)||defined(CONFIG_PROJECT_V3802))
...
{
    int ret = 0;
	 APS_LOG("epl_sensor_I2C_Write MDP  regaddr= %x  data=%x \r\n",regaddr,data);
...
static int epl_sensor_I2C_Read(struct i2c_client *client, uint8_t regaddr, uint8_t bytecount)
{
    ...
    int read_count=0, rx_count=0;
	 APS_LOG("epl_sensor_I2C_Read MDP  regaddr= %x  bytecount=%x \r\n",regaddr,bytecount);
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
...
#ifdef ALS_TINNO_DYN
	epl_sensor.als.integration_time = 0x18;
    	epl_sensor.als.gain = 0x03;	
#endif
    ...
    #if defined(CONFIG_PROJECT_V2600)
    dynk_low_offset =300;               //ftm modify ps  60
    dynk_high_offset =1300;			   //ftm modify ps 180
	#elif (defined(CONFIG_PROJECT_V3800)||defined(CONFIG_PROJECT_V3802)||defined(CONFIG_PROJECT_V3610))
    dynk_low_offset =350;               //ftm modify ps  60
    dynk_high_offset =1200;
        #elif (defined(CONFIG_PROJECT_V3760))
    dynk_low_offset =950;               //ftm modify ps  60
    dynk_high_offset =1800;    
	#else
    dynk_low_offset =140;               //ftm modify ps  60
    dynk_high_offset =260;
    #endif
#endif /*PS_DYN_K*/
...
static int tinno_get_als_value(struct epl_sensor_priv *obj, u16 als)
{
 int idx;
 
    for(idx = 0; idx < 16; idx++)
            {
                if(als < tinno_als_level[idx])
                {
                    break;
                }
            }
	...
	APS_LOG("step_value:anti_count =  %d orient_flag=%d \n",anti_count,orient_flag);
					...
					switch(orient_index)
						{
							case 0: 	step_down_flag=1; 
									return tinno_als_value[0];
							case 1: 	step_down_flag=1; 
									return tinno_als_value[1]; 
							case 2: 	opp_anti= 2; 
				...
				if(orient_flag==0)
				{
					...
					switch(orient_index)
					{
						...
						case 3: 	step_up_flag=1;
								return tinno_als_value[3];
						case 4: 	step_up_flag=1;
								return tinno_als_value[4];
						default : opp_anti= 2; 
								APS_LOG("step_value:case default=  %d \n",opp_anti);
							break;
					}
		...
		{	
		   ...
		   orient_flag=0;
                ...
                APS_DBG("niu anti ALS: idx = %d, %05d => %05d\n",idx, als, tinno_als_value[idx]);
                return tinno_als_value[idx];
            }
            else
            {
                APS_ERR("ALS: %05d => %05d (-1)\n", als, tinno_als_value[idx]);
                return als;
            }
}
...
static int epl_sensor_get_als_value(struct epl_sensor_priv *obj, u16 als)
{
    ...
    switch(epl_sensor.als.report_type)
    {
		...
		case CMC_BIT_DYN_INT:
            ...
            {
            #ifdef ALS_TINNO_DYN
	           	als_value_tinno= tinno_get_als_value(obj,dynamic_intt_lux);
			printk("niu anti als_value_tinno = %d \n",als_value_tinno);
			return als_value_tinno;
		#endif
...
static void epl_sensor_report_ps_status(void)
{
    ...
    APS_LOG("[%s]: epl_sensor.ps.high_threshold=%d, ps.low_threshold=%d \r\n", __func__, epl_sensor.ps.high_threshold, epl_sensor.ps.low_threshold);
...
void epl_sensor_enable_ps(int enable)
{
    ...
    if(enable_ps != enable)
    {
	    ...
	    //set_psensor_intr_threshold(dynk_thd_low, dynk_thd_high); 
            //dynk_min_ps_raw_data=epl_sensor.ps.data.data+10;
...
void epl_sensor_dynk_thd_polling_work(struct work_struct *work)
{
...
#endif
    {
        ...
        if(enable_ges == true && epl_sensor.ges.polling_mode == 1)
        {
			...
			APS_LOG("[%s]:als / ps enable: %d \n", __func__,epl_sensor.ps.data.data);
#if 1//defined(CONFIG_PROJECT_V3710)
        #if (defined(CONFIG_PROJECT_V3800)||defined(CONFIG_PROJECT_V3802)||defined(CONFIG_PROJECT_V3610))
		if((epl_sensor.ps.data.data > g_ps_base_value+2500)&&(ps_smart_gesture_flag == 0))
	 #else	
		if((epl_sensor.ps.data.data > g_ps_base_value+5000)&&(ps_smart_gesture_flag == 0))
	 #endif
		{
			set_psensor_intr_threshold(g_ps_base_value+2000, g_ps_base_value+4800);
			schedule_delayed_work(&obj->dynk_thd_polling_work, msecs_to_jiffies(dynk_polling_delay));
			return;
		
		}
	ps_smart_gesture_flag = 1;
	 ...
	 #ifdef PS_TINNO_DYN
		if(g_ps_base_value>epl_sensor.ps.data.data)
			dynk_min_ps_raw_data=g_ps_base_value;
		
	#endif
	
            if(enable_ps == true)
            { 
            #ifdef CONFIG_PROJECT_V2600
            	if(dynk_min_ps_raw_data<0x400)
            	{
            		if(dynk_min_ps_raw_data<0x100)
            		{ 
            			dynk_thd_low = dynk_min_ps_raw_data + dynk_low_offset;
		    		    dynk_thd_high = dynk_min_ps_raw_data + (dynk_high_offset/3);
					}
					else
					{
		                dynk_thd_low = dynk_min_ps_raw_data + dynk_low_offset;
					...
					if(dynk_thd_low>(dynk_thd_high+1))
					{
							dynk_thd_high=dynk_thd_low+150;
					}
...
#if defined  CONFIG_PROJECT_V3702
			if(g_ps_cali_flag && g_ps_base_value >0 && g_ps_base_value < 450){
						...
						{ 
							...
							dynk_thd_low =	dynk_min_ps_raw_data + 45 ;// dynk_min_ps_raw_data + dynk_low_offset;
							dynk_thd_high = dynk_min_ps_raw_data + 80;//dynk_min_ps_raw_data + dynk_high_offset;
...
static void epl_sensor_eint_work(struct work_struct *work)
{
   ...
   enable_irq_wake(epl_sensor_obj->irq);
...
*/
	   ...
	   epl2570_alspsPltFmDev = get_alsps_platformdev();
	/* gpio setting */
		pinctrl = devm_pinctrl_get(&epl2570_alspsPltFmDev->dev);
		if (IS_ERR(pinctrl)) {
			ret = PTR_ERR(pinctrl);
	...
	if (epl_sensor_obj->irq_node) {
		...
		APS_LOG("epl2570_obj->irq = %d\n", epl_sensor_obj->irq);
		if (!epl_sensor_obj->irq) {
			APS_ERR("irq_of_parse_and_map fail!!\n");
			return -EINVAL;
		}
		if (request_irq
		    (epl_sensor_obj->irq, epl_eint_handler, IRQF_TRIGGER_NONE, "ALS-eint", NULL)) {
...
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_store_als_gain(struct device_driver *ddri, const char *buf, size_t count)
{
	struct epl_sensor_priv *epld = epl_sensor_obj;
	int value = 0;
...
static ssize_t epl_sensor_store_als_inte(struct device_driver *ddri, const char *buf, size_t count)
{
	struct epl_sensor_priv *epld = epl_sensor_obj;
	int value = 0;
...
#ifndef MDP
            epl_sensor.als.integration_time = value<<2;
	        epl_sensor_I2C_Write(epld->client, 0x01, epl_sensor.als.integration_time | epl_sensor.als.gain);
#else
	switch (epl_sensor.mode)
	{
...
#ifndef MDP
APS_LOG("epl_sensor_store_ir_drive MDP    *buf= %x  value=%x epl_sensor.mode=%d\r\n",*buf,value,epl_sensor.mode);
...
static DRIVER_ATTR(als_integration,					S_IWUSR | S_IRUGO, NULL,								epl_sensor_store_als_inte				);


static DRIVER_ATTR(gain,					    S_IWUSR | S_IRUGO, NULL,								epl_sensor_store_gain					    );
static DRIVER_ATTR(als_gain,					    S_IWUSR | S_IRUGO, NULL,								epl_sensor_store_als_gain					    );
...
static DRIVER_ATTR(chipinfo,     				S_IWUSR |   S_IRUGO, show_chipInfo, NULL);
...
static void ps_cali_start(void)
{
    ...
    if(NULL == epl_sensor_obj->client)
    {
        APS_ERR("epl_sensor_obj->client == NULL\n");
        return;
    }
    ...
    */
	...
	/* if(err = ltr553_read_data_ps(epl_sensor_obj->client, &epl_sensor_obj->ps))
	  {
			APS_ERR("read data error\n");
	  }
        vl_read_ps = epl_sensor_obj->ps;*/

		vl_read_ps = factory_ps_data();
...
exit_handle:
    ...
    /*
    if((err = enable_ps(epl_sensor_obj->client, 0)))
    {
...
static void epl2570_ps_cali_set_threshold(void){
...
*/
  //  client->timing = 400;
    ...
    epl_sensor_obj = obj;
 	 ...
 	 printk("%s:111 client->addr=%d\n", __func__,client->addr);
 printk("%s:geroge   hw->i2c_addr=%d\n", __func__,hw->i2c_addr);
  obj->hw = get_cust_alsps_hw();
  printk("%s:geroge 22  hw->i2c_addr=%d\n", __func__,hw->i2c_addr);
	client->addr=0x49;//obj->hw->i2c_addr[0];//zhiwei add for i2c addr from dts
printk("%s: 222 client->addr=%d\n", __func__,client->addr);
	 obj->hw = hw;
...
{
    struct alsps_hw *hw = epl2570_get_cust_alsps_hw();
...
static int __init epl_sensor_init(void)
{
    ...
    APS_FUN();
	const char *name = "mediatek,EPL2570";

	hw =   get_alsps_dts_func(name, hw);
	  printk("%s: geroge i2c_number=%d\n", __func__,hw->i2c_num);
	  printk("%s:geroge i2c_addr=%d\n", __func__,hw->i2c_addr);
	  
    if(alsps_driver_add(&epl_sensor_init_info)){
		APS_ERR("add driver error\n");
		return -1;
	}
...
MODULE_DESCRIPTION("EPL2570 ALPsr driver");
MODULE_LICENSE("GPL");
