...
int _esd_check_config_handle_vdo(void)
{
	...
	/* pull DSI clock lane */
        #if defined(CONFIG_PROJECT_V2610)
        if(gEnable_DSI_ClkTrail){
        #endif
...
int primary_display_esd_check(void)
{
	...
	if (pgc->plcm->params->dsi.customization_esd_check_enable == 0) {
		MMProfileLogEx(ddp_mmp_get_events()->esd_extte,
                ...
                #if defined(CONFIG_PROJECT_V2610)
		if (primary_display_is_video_mode() && gEnable_DSI_ClkTrail) 
                #else
                if (primary_display_is_video_mode())
                #endif
                {
...
unsigned long get_dim_layer_mva_addr(void)
{
	if (dim_layer_mva == 0) {
     ...
     // TINNO BEGIN
     // Modified by leaf.zhang on Nov. 14rd , 2016 for DHACN-769.MTK patch has not been released yet.
     #if 0
