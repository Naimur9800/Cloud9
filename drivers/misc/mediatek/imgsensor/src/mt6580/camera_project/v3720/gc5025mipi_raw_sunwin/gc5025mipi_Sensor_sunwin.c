...
#define GC5025_SUNWIN_DEFAULT_DUMMY_PIXEL_NUMS   0x25c /* HB */
#define GC5025_SUNWIN_DEFAULT_DUMMY_LINE_NUMS    0x08 /* VB */
static kal_uint32 Basic_Dummy_Line = GC5025_SUNWIN_DEFAULT_DUMMY_LINE_NUMS;
/****************************Modify Following Strings for Debug****************************/
#define PFX "GC5025_SUNWIN_camera_sensor"
...
static int gc5025_gcore_check_version(void)
{
	...
	gc5025_select_page_otp(otp_page1);
	if(gc5025_read_otp(0x26)==0x01)
		DR_State = KAL_FALSE;
	else
		DR_State = KAL_TRUE;
	if((gc5025_read_otp(0x27)&0x01) == 0x01) {
		LOG_INF("GC5025A sensor! \n");
	}
	gc5025_select_page_otp(otp_page0);
	if(((gc5025_read_otp(0x7f)&0x07) == 0x02)&&(DR_State == KAL_FALSE)) {
		LOG_INF("COB packaged with error test version! \n");
		DR_State = TRUE;
	}
	if((gc5025_read_otp(0x7f)&0x07) == 0x01) {
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    kal_uint8 retry = 2;
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			if(gc5025_otp_info.module_id == 0x06){
...
UINT32 GC5025MIPI_SUNWIN_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
