...
//#undef FEATURE_REQS_UNIFY   //for test
...
// FEATURE_REQS_UNIFY, lejun.wang, DATE20161018, for wiko unify
#ifdef FEATURE_REQS_UNIFY
void _get_md_postfix(int md_id, char k[], char buf[], char buf_ex[])
#else
...
void get_md_postfix(int md_id, char k[], char buf[], char buf_ex[])
{
    ...
    if (buf_ex){
        ...
        printk("==========get_md_postfix buf_ex=%s==========\n",buf_ex);
    }
...
#define MAX_REMAP_SIZE (1024 * 1024)
    ...
    if (img->type == IMG_MD) {    /*  Gen MD image name */
        ...
        // TINNO BEGIN
        // FEATURE_REQS_UNIFY, lejun.wang, DATE20161018, for wiko unify
        printk("ccci_load_firmware modem img_name=%s\n",img_name);
...
TRY_LOAD_IMG:
    ...
    if (ret != 0) {
        ...
        if (i <= scan_max) {
            ...
            if (img->type == IMG_MD){
                #ifdef FEATURE_REQS_UNIFY
                snprintf(img_name, IMG_NAME_LEN, "modem_%d_%s_n_%s.img", md_id + 1, type_str[i], Market_Area);
                printk("ccci_load_firmware request_firmware modem img_name=%s\n",img_name);
