...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		...
		if (((g_temp_status == TEMP_POS_45_TO_POS_60)
			...
			/***caizhifu mark start, please check battery temp in bat_thread_kthr work, mark the log, 2016-11-11****/
			//battery_log(BAT_LOG_CRTI,
			//	    "[BATTERY] Battery Normal Temperature between %d and %d !!\n\r",
			//	    TEMP_POS_10_THRESHOLD, TEMP_POS_45_THRESHOLD);
			/***caizhifu mark end, please check battery temp in bat_thread_kthr work, mark the log, 2016-11-11****/
			g_temp_status = TEMP_POS_10_TO_POS_45;
			if (batt_cust_data.high_battery_voltage_support)
				g_jeita_recharging_voltage = 4280;		//caizhifu modified (4200->4280), recharging slow,and soc jump from 100% to 99% when the charger is connecting, 2016-10-07
			else
...
//TINNO add begin
      if(g_temp_status == TEMP_POS_45_TO_POS_60){
			...
			// TINNO BEGIN
			// Added by liaoye on Dec. 29, 2016 for set jeita charge current
...
// Added by liaoye on Dec. 29, 2016 for set jeita charge current
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
	pr_debug("%s: disable bcct, use jeita standard \n", __func__);
	return 0;
#else
...
//TINNO add CHRGE_REAL_CURRENT_NON_STD
#ifdef CHRGE_REAL_CURRENT_NON_STD


		  if(g_temp_standar_charger_value > TINNO_NON_STD_AC_CHARGER_CURRENT)
			{
			
				g_temp_CC_value = TINNO_NON_STD_AC_CHARGER_CURRENT;
			}
			else if((g_temp_standar_charger_value <= TINNO_NON_STD_AC_CHARGER_CURRENT) && (g_temp_standar_charger_value > MIN_AC_CHARGER_CURRENT))
			{
...
//TINNO add CHRGE_REAL_CURRENT
#ifdef CHRGE_REAL_CURRENT

			g_temp_CC_value =MIN_AC_CHARGER_CURRENT;// batt_cust_data.ac_charger_current;//
			
			if(g_temp_standar_charger_value > MAX_AC_STANDARD_CHARGER_CURRENT)
			{
				g_temp_CC_value = MAX_AC_STANDARD_CHARGER_CURRENT;
			}
			else if((g_temp_standar_charger_value <= MAX_AC_STANDARD_CHARGER_CURRENT) && (g_temp_standar_charger_value > MIN_AC_CHARGER_CURRENT))
			{
				g_temp_CC_value = g_temp_standar_charger_value;
			}
        ...
        else if (BMT_status.charger_type == TINNO_1_5A_CHARGER) 
        {
		...
		#ifdef CHRGE_REAL_CURRENT //yixuhong 20151116 add for test
			if(g_temp_standar_charger_value > MAX_AC_TINNO_1_5A_CHARGER_CURRENT)
			{
				g_temp_CC_value = MAX_AC_TINNO_1_5A_CHARGER_CURRENT; 
			}
			else if((g_temp_standar_charger_value <= MAX_AC_TINNO_1_5A_CHARGER_CURRENT) && (g_temp_standar_charger_value > MIN_AC_CHARGER_CURRENT))
			{
				g_temp_CC_value = g_temp_standar_charger_value;
			}
			...
			g_temp_CC_value = batt_cust_data.tinno_1_5a_charger_current;
		#endif
		...
		battery_log(BAT_LOG_CRTI, "g_temp_standar_charger_value: %d charger_type:%d  Default CC mode chg_current: %d\r\n",
			    g_temp_standar_charger_value, BMT_status.charger_type, g_temp_CC_value);
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		battery_xlog_printk(BAT_LOG_CRTI, "maxPgFlag=%d g_temp_status=%d g_temp_CC_value=%d \r\n",maxPgFlag,g_temp_status,g_temp_CC_value);
#else


#ifdef CONFIG_PROJECT_V250X

battery_xlog_printk(BAT_LOG_CRTI, "[geroge] 777 BMT_status.temperature = %d \n",BMT_status.temperature);
if(BMT_status.temperature > 5 && BMT_status.temperature <= 10)
		g_temp_CC_value = AC_CHARGER_TEMP_0_10_CURRNET;   //CHARGE_CURRENT_300_00_MA,300ma charge

if(BMT_status.temperature >= 0 && BMT_status.temperature <= 5)
		g_temp_CC_value = 20000;  //CHARGE_CURRENT_200_00_MA ,  200ma charge
