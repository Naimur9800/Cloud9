	...
	if(enable)
	{
		...
		else
		{
			...
			{			
				...
				APS_LOG("%s: stk3x1x_get_ps_value_only:0x%x \n",__FUNCTION__,err);
				if(err < 0)
				{
...
static int stk_ps_tune_zero_final(struct stk3x1x_priv *obj)
{
	...
	APS_LOG("%s: obj->ps_stat_data=%d, ps_low_thd_val=%d\n", __func__, obj->ps_stat_data[1]);
#ifndef CALI_EVERY_TIME
	...
	APS_LOG("%s: ps_high_thd_val=%d, ps_low_thd_val=%d\n", __func__, obj->ps_high_thd_val, obj->ps_low_thd_val);
#else						
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(ret == 0)
	{
		...
		APS_LOG("%s: update psa:word_data=%d, psa=%d,psi=%d\n", __func__, word_data,obj->psa, obj->psi);
		...
		if(word_data > g_ps_base_value + 200)
		{
			APS_LOG("%s:word_data=%d, g_ps_base_value=%d\n", __func__, word_data,g_ps_base_value);
			obj->psi = g_ps_base_value;
			diff = obj->psi;
		}
	...
	APS_LOG( "%s: diff=0x%x,obj->psi=0x%x\n", __func__,diff, obj->psi);
	if(diff == obj->psi)
	{
		//obj->psi_set = obj->psi;
		...
		APS_LOG( "%s: obj->ps_high_thd_val:0x%x,obj->ps_high_thd_tmp:0x%x\n", __func__,obj->ps_high_thd_val,obj->ps_high_thd_tmp);
...
static void stk_ps_tune0_work_func(struct work_struct *work)
{
	...
	APS_LOG("%s: obj->tune_zero_init_proc=%d\n", __func__, obj->tune_zero_init_proc);
