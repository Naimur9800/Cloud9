...
struct pinctrl_state 		*mas_finger_power1v8_on, *mas_finger_power1v8_off,	//power1v8
 ...
 */
 #if POWER_VGP
 	reg = regulator_get(&pdev->dev, "vgp1");
    ret = regulator_set_voltage(reg, 2800000, 2800000);
    if(ret){
        MALOGE("regulator_set_voltage error\n ");
        return -1;
    }
 #elif POWER_GPIO
 	mas_finger_power2v8_on = pinctrl_lookup_state(mas_finger_pinctrl, "fingerprint_power_high");
	if (IS_ERR(mas_finger_power2v8_on)) {
 	...
 	mas_finger_power2v8_off = pinctrl_lookup_state(mas_finger_pinctrl, "fingerprint_power_low");
 ...
 #endif
	mas_finger_power1v8_on = pinctrl_lookup_state(mas_finger_pinctrl, "fingerprint_reset_high");
	...
	mas_finger_power1v8_off = pinctrl_lookup_state(mas_finger_pinctrl, "fingerprint_reset_low");
...
/*
 * this kernel do not need to set spi gpio
...
*/
	mas_finger_eint_on = pinctrl_lookup_state(mas_finger_pinctrl, "fingerprint_pin_irq");
	if (IS_ERR(mas_finger_eint_on)) {
		ret = PTR_ERR(mas_finger_eint_on);
