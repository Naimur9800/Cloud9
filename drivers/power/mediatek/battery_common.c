...
/* ///////////////////////////////////////////////////////////////////////////////////////// */
#define RECOVERY_CHARGING_VOLTAGE 6300
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ISenseVoltage:
		//caizhifu modified start for charger voltage update slow, 2016-12-19
		//val->intval = data->BAT_ISenseVoltage;
		val->intval = battery_meter_get_VSense();
		//caizhifu modified end for charger voltage update slow, 2016-12-19
		break;
	case POWER_SUPPLY_PROP_ChargerVoltage:
		//caizhifu modified start for charger voltage update slow, 2016-12-19
		//val->intval = data->BAT_ChargerVoltage;
		val->intval = battery_meter_get_charger_voltage();
		//caizhifu modified end for charger voltage update slow, 2016-12-19
		break;
		/* Dual battery */
...
static kal_bool mt_battery_100Percent_tracking_check(void)
{
	...
	if (BMT_status.bat_full == KAL_TRUE) {	/* charging full first, UI tracking to 100% */
		...
		if ((BMT_status.UI_SOC - BMT_status.SOC) > 25 && is_uisoc_ever_100 == KAL_TRUE) {
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
			    ...
			    || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
		{
			...
			if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||
				...
				||(BMT_status.charger_type == APPLE_2_1A_CHARGER)  
				||(BMT_status.charger_type == TINNO_1_5A_CHARGER)
	#endif
				) 
#else
			if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||
...
#if defined(CONFIG_TINNO_QUICK_CHARGING)
   #if defined(CONFIG_TINNO_NON_STD_AC_CHARGER_PG)
   #define MAX_CHARGING_PG 1200000
   #else
   #define MAX_CHARGING_PG 900000
   #endif
#if defined(QUICK_CHARGINT_MAX_CHARGING_CURRENT)
#define MAX_CHARGING_CURRENT QUICK_CHARGINT_MAX_CHARGING_CURRENT
#else
#define MAX_CHARGING_CURRENT 1300
#endif
#define MAX_CHARGER_VOL 6000
unsigned int maxPgFlag = 0;
int batteryAveragePg=0;
int batteryPgCounterIndex = 0;
int batteryPgCounterIndex2 = 0;
extern CHR_CURRENT_ENUM g_temp_standar_charger_value;//yixuhong add for test
#endif

//liaoye, fixed bugid: DHACN-857
...
void mt_battery_GetBatteryData(void)
{
	...
	int pg, v;
	static int pg_sum;
	static int batteryPgBuffer[BATTERY_AVERAGE_SIZE];
	static unsigned char batteryPgIndex = 0;
#endif
...
//liaoye@2016/11/04, update the UI_SOC immediately after system resume.
#if defined(SOC_BY_SW_FG)
		if( BMT_status.charger_exist == KAL_FALSE && bat_spm_timeout == TRUE ) if(SOC<BMT_status.UI_SOC) BMT_status.UI_SOC = SOC;
#endif
...
#if defined(CONFIG_TINNO_QUICK_CHARGING)
	v = (charger_vol - Vsense); 
	pg = v*ICharging;
	g_temp_standar_charger_value = MAX_CHARGING_PG/v*100;
	
	if((ICharging>0)&(v>0)&(pg>1000))
	{
	    batteryAveragePg=mt_battery_average_method(BATTERY_AVG_PQ,&batteryPgBuffer[0], pg, &pg_sum, batteryPgIndex);
	    batteryPgIndex++;
                 if (batteryPgIndex >= BATTERY_AVERAGE_SIZE) batteryPgIndex = 0;
	}
             
	if((batteryAveragePg>=MAX_CHARGING_PG) || 
	   (ICharging>MAX_CHARGING_CURRENT)    || 
	   (charger_vol >= MAX_CHARGER_VOL))
	{
	    if((maxPgFlag>=1) && (batteryPgCounterIndex>=BATTERY_AVERAGE_SIZE))
	    {
                        maxPgFlag = 2;
                        batteryPgCounterIndex2++;
                        if (batteryPgCounterIndex2 >= BATTERY_AVERAGE_SIZE)
                        {
                                maxPgFlag = 3;
                                batteryPgCounterIndex2 = BATTERY_AVERAGE_SIZE;
                        }
	    }	
                else
                {
                        maxPgFlag = 1;
                        batteryPgCounterIndex++;
                        if (batteryPgCounterIndex >= BATTERY_AVERAGE_SIZE)
                        batteryPgCounterIndex = BATTERY_AVERAGE_SIZE;
                }
	}
	else
	{//reset power protect
		maxPgFlag = 0;
		batteryPgCounterIndex = 0;
...
//liaoye, fixed bugid: DHACN-857, start
	if (BMT_status.charger_exist == KAL_TRUE && gFG_Is_Charging == KAL_TRUE) {
		if (BMT_status.SOC < BMT_status.UI_SOC)
			BMT_status.SOC = BMT_status.UI_SOC;
	}
//liaoye, fixed bugid: DHACN-857, end
...
//caizhifu modified start for reduce log printk in driver,add some log to work thread, 2016-11-11
#if defined(CONFIG_TINNO_QUICK_CHARGING)
	battery_log(BAT_LOG_CRTI,
	"AvgVbat=(%d,%d),AvgI=(%d,%d),VChr=%d,AvgT=(%d,%d),SOC=(%d,%d),UI_SOC=%d,ZCV=%d,CHR_Type=%d bcct:%d:%d I:%d Ibat:%d maxPgFlag:%d GTEMP_CHG_V:%d V:%d batteryAveragePg=%d, pg=%d\n",
		    ...
		    get_bat_charging_current_level(), BMT_status.IBattery / 10,maxPgFlag, g_temp_standar_charger_value,v,\
		    batteryAveragePg, pg);
#else
	battery_log(BAT_LOG_CRTI,
...
#endif
//caizhifu modified end for reduce log printk in driver,add some log to work thread, 2016-11-11
}
...
static PMU_STATUS mt_battery_CheckBatteryTemp(void)
{
	...
	battery_log(BAT_LOG_FULL, "[BATTERY] support JEITA, temperature=%d\n",
		    BMT_status.temperature);
...
{
//caizhifu add start for battery event change when cable out, 2016-10-31
	struct battery_data *bat_data = &battery_main;
	struct power_supply *bat_psy = &bat_data->psy;
//caizhifu add end for battery event change when cable out, 2016-10-31
...
*/
		...
		power_supply_changed(bat_psy); //caizhifu add for battery event change when cable out, 2016-10-31
#ifdef CONFIG_MTK_BQ25896_SUPPORT
/*New low power feature of MT6531: disable charger CLK without CHARIN.
* MT6351 API abstracted in charging_hw_bw25896.c. Any charger with MT6351 needs to set this.
* Compile option is not limited to CONFIG_MTK_BQ25896_SUPPORT.
...
void check_battery_exist(void)
{
#if 1//defined(CONFIG_DIS_CHECK_BATTERY)	//caizhifu remove for the factory test, 2016-11-10
...
static struct dentry *scc_dentry;
 void mtkbchg_init_speed_current_map(void){
	struct device_node *node;
	int rc = 0;

	node = of_find_compatible_node(NULL, NULL, "mediatek,battery");
	if (node) {
		rc = of_property_read_u32_array(node, "speed-current", batt_cust_data.speed_current_map, SPEED_MAX);
		if (rc) {
			memset(batt_cust_data.speed_current_map,0,sizeof(batt_cust_data.speed_current_map));
			battery_log(BAT_LOG_CRTI,"Couldn't read speed-current rc = %d\n", rc);
		}
	} else {
		battery_log(BAT_LOG_CRTI,"Couldn't read speed-current rc = %d\n", rc);
		memset(batt_cust_data.speed_current_map,0,sizeof(batt_cust_data.speed_current_map));
	}
	batt_cust_data.target_speed = 0;
	batt_cust_data.charge_speed = 0;
}
/*scc end*/
...
/*scc begin add by sunhonghui*/
	mtkbchg_init_speed_current_map();
/*scc end*/
				    ...
				    const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	batt_cust_data.charge_speed = speed;
	if(speed != batt_cust_data.target_speed) {
		batt_cust_data.target_speed = speed;
		bat_thread_wakeup();
	}
...
/*scc begin add by sunhonghui*/
	scc_dentry = debugfs_create_file("scc",S_IRUGO, NULL, NULL, &scc_fops);
/*scc end*/
