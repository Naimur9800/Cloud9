...
//68m
const u32 CS_VTH[] = {
	CHARGE_CURRENT_500_00_MA,CHARGE_CURRENT_600_00_MA,
	CHARGE_CURRENT_650_00_MA, CHARGE_CURRENT_700_00_MA, CHARGE_CURRENT_900_00_MA,
	    CHARGE_CURRENT_1100_00_MA,CHARGE_CURRENT_1300_00_MA,CHARGE_CURRENT_1500_00_MA
...
static u32 charging_hw_init(void *data)
{
	...
	battery_log(BAT_LOG_CRTI, "[charging_hw_init] set bq24157 initial value.\n");
	...
	if (!charging_init_flag) {
		bq24157_reg_config_interface(0x04, 0x5A);	/* 146mA */
...
static u32 charging_set_cv_voltage(void *data)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq24157] charging_set_cv_voltage =%d register_value=%d\n",*(u32 *) (data),register_value);
...
static u32 charging_set_input_current(void *data)
{
	...
	if (*(u32 *) data > CHARGE_CURRENT_500_00_MA) {
        ...
        printk("24157 charging_set_current = %d \r\n",register_value);
...
static u32 charging_get_charging_status(void *data)
{
	...
	ret_val = bq24157_get_chip_status();
     printk("24157 charging_get_charging_status = %d \r\n",ret_val);
	if (ret_val == 0x2)
		*(u32 *) data = KAL_TRUE;
	else
		*(u32 *) data = KAL_FALSE;
