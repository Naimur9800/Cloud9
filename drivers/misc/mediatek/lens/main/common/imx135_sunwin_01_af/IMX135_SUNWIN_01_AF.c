...
#define AF_DRVNAME "IMX135_SUNWIN_01_AF_DRV"
#define AF_I2C_SLAVE_ADDR        0x18

#define AF_DEBUG
#ifdef AF_DEBUG
//#define LOG_INF(format, args...) pr_debug(AF_DRVNAME " [%s] " format, __func__, ##args)
#define LOG_INF(format, args...) pr_err(AF_DRVNAME "[%s] " format, __FUNCTION__, ##args)
#else
...
//update by DW diver ic test data
static void initdrv(void)
{
	char puSendCmd2[2] = {0x01,0x39};//Set SAC3mode
	char puSendCmd3[2] = {0x05,0x08};//Set Tvib(DIV[1:0],SACT[5:0])
...
//	printk("bo_liu, dw9714 move [curr] %d [target] %d\n", g_u4CurrPosition, g_u4TargetPosition);
...
/* ////////////////////////////////////////////////////////////// */
long IMX135_SUNWIN_01_AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	long i4RetValue = 0;
...
*/
//add for AF voice issue
static void AF_CloseDelay(void)
{
    int cur_position = 300;
    int last_position = 150;
    int back_step = 30;
...
int IMX135_SUNWIN_01_AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
...
void IMX135_SUNWIN_01_AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
