...
//#define OV8856_OTP_TEST_KEY

//#define OV8856R1AOTP

#define PFX "sv8856camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.linelength  = 2417,				//record different mode's linelength
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			LOG_INF("Read sensor id fail, id: 0x%x,sensor_id = 0x%d\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
