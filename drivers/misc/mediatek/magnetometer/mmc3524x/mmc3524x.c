...
#define MMCDBG(format, ...)	printk(KERN_ERR "mmc3524x " format "\n", ## __VA_ARGS__)
#else
...
struct mag_hw mag_cust_formmc;
static struct mag_hw *hw = &mag_cust_formmc;
...
static DEFINE_MUTEX(mmc3524x_i2c_mutex);
...
static int mag_i2c_read_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
	...
	if (!client) {
		mutex_unlock(&mmc3524x_i2c_mutex);
		return -EINVAL;
 ...
 */
int TEST_DATA_formmc(const char testno[], const char testname[], const int testdata,
...
/*!
 ...
 @retval 0 The test is aborted by kind of system error.
 */
int FctShipmntTestProcess_Body_formmc(void)
{
	int pf_total = 1;
	...
	/* Reset Test Result */
	/* *********************************************** */
	TEST_DATA_formmc(NULL, "START", 0, 0, 0, &pf_total);
	...
	/* Judge Test Result */
	/* *********************************************** */
	TEST_DATA_formmc(NULL, "END", 0, 0, 0, &pf_total);
...
static ssize_t show_shipment_test(struct device_driver *ddri, char *buf)
{
	...
	res = FctShipmntTestProcess_Body_formmc();
	if (1 == res) {
