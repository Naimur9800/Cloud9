 ...
 * configuration
*******************************************************************************/
#define I2C_DRIVERID_ltr553 559
/*----------------------------------------------------------------------------*/
#define ltr553_I2C_ADDR_RAR 0 /*!< the index in obj->hw->i2c_addr: alert response address */
#define ltr553_I2C_ADDR_ALS 1 /*!< the index in obj->hw->i2c_addr: ALS address */
#define ltr553_I2C_ADDR_PS  2 /*!< the index in obj->hw->i2c_addr: PS address */
...
static void ps_cali_tp_check(void);
...
static int ltr553_dynamic_calibrate(void);
#endif
...
static int ltr553_alsps_remove(void)
{
//    struct alsps_hw *hw = get_cust_alsps_hw();
    APS_FUN();
    ltr553_power(hw, 0);
...
static int lux_formula(int ch0_adc, int ch1_adc, u8 eqtn)
{
    ...
    if (hwmsen_read_byte_sr(ltr553_obj->client, APS_RO_ALS_PS_STATUS, &buf))
    {
    ...
    if (hwmsen_read_byte_sr(ltr553_obj->client, APS_RW_ALS_MEAS_RATE, &buf))
    {
...
#define RATIO_HYSVAL    10
    ...
    if (hwmsen_read_byte_sr(ltr553_obj->client, APS_RW_ALS_CONTR, &buf))
    {
...
static int set_low_temperature_threshold_value(struct ltr553_priv *obj,int ps)
{
	int temp_value_high;
	int temp_value_low;
	//struct alsps_hw *hw = get_cust_alsps_hw();

	APS_LOG(" reset_low_temperature_threshold_value:  ps=%d \n",ps);
	APS_LOG("reset_low_temperature_threshold_value:reset_flag_low_temperature= %d  set_first_value_flag_low_temperature=%d \n", reset_flag_low_temperature,set_first_value_flag_low_temperature);
	...
	APS_LOG("set_low_temperature_threshold_value:g_ps_base_value= %d \n", g_ps_base_value);
	APS_LOG(" set_low_temperature_threshold_value:readobj->ps_thd_val_high=%x, obj->ps_thd_val_low=%x! \n",atomic_read(&obj->ps_thd_val_high), atomic_read(&obj->ps_thd_val_low));
	...
	if(set_first_value_flag_low_temperature==0)
	{
        if(ps > 0x600)
        {
		atomic_set(&ltr553_obj->ps_thd_val_high, 0x620);
        atomic_set(&ltr553_obj->ps_thd_val_low,  0x5a0);
            return ps;
        }
		if(g_ps_base_value+50<ps)
		{	

			APS_LOG(" set_low_temperature_threshold_value:PS is larger than g_ps_base_value, and threshold  need to be reset! \n");
			APS_LOG(" set_low_temperature_threshold_value: ltr553  default_ps_highthresh=%x, default_ps_lowthresh=%x! \n",atomic_read(&obj->ps_thd_val_high), atomic_read(&obj->ps_thd_val_low));
		...
		else
		{
			...
			set_first_value_flag_low_temperature=1;
                                        atomic_set(&ltr553_obj->ps_thd_val_high, temp_value_high);
                                        atomic_set(&ltr553_obj->ps_thd_val_low,  temp_value_low);
			APS_LOG(" set_low_temperature_threshold_value:There is no need to set threshold! \n");
	...
	if(temp_value_high> 0x7ff)
	{
		...
		APS_LOG("set_low_temperature_threshold_value: set value_high=0x7ff,value_low=0x7ff, please check the phone \n");
	}
	
	APS_LOG(" set_low_temperature_threshold_value: ltr553  temp_value_high=%x, temp_value_low=%x! \n",temp_value_high,temp_value_low);
...
int ltr553_init_device(struct i2c_client *client)
{
    ...
    hwmsen_write_byte(client, 0x9e, 0x00);              ///2 consecutive data outside range to interrupt
    if(part_id == ltr553_PART_ID)
    {
        hwmsen_write_byte(client, 0x84,0x00 );          //0x0f ps measurement time 10 ms 0x01=70ms
        als_range = ltr553_ALS_ON_Range8;
        ps_gain = ltr553_PS_ON_Gain16;
    }
    ...
    {
        APS_ERR("ltr553 part_id is error, part_id = 0x%x\n", part_id);
    }
	APS_ERR("LTR553 part_id = 0x%x\n", part_id);
#ifdef GN_MTK_BSP_ALSPS_INTERRUPT_MODE
...
int ltr553_init_device_ESD_Recover(struct i2c_client *client)
{
	int err = 0;
...
static int ltr553_enable_ps(struct i2c_client *client, bool enable)
{
   ...
   struct hwm_sensor_data sensor_data;
    ...
    else if (enable == true)
    {
	    ...
	    if((err = ltr553_init_device_ESD_Recover(client)))
	    {
	        APS_ERR("init dev: %d\n", err);
	        return err;
	    }
        ...
        {
            APS_LOG("ltr553_enable_ps enable failed!\n");
            return -1;
        }
        hwmsen_read_byte_sr(client, APS_RW_PS_CONTR, &regdata);
        APS_ERR("ltr553_enable_ps, regdata: %0xx!\n", regdata);
    ...
    else if(enable == false)
    {
        ...
        {
            APS_LOG("ltr553_enable_ps disable failed!\n");
            return -1;
        }
        hwmsen_read_byte_sr(client, APS_RW_PS_CONTR, &regdata);
        APS_ERR("ltr553_enable_ps, regdata: %0xx!\n", regdata);
...
/*----------------------------------------------------------------------------*/
#ifdef GN_MTK_BSP_ALSPS_INTERRUPT_MODE
static void ltr553_eint_func(void)
{
...
static void ltr553_eint_work(struct work_struct *work)
{
  ...
  struct  hwm_sensor_data sensor_data;
    ...
    APS_ERR("ltr553_eint_work obj->pending_intr =%lu, obj:%p\n",obj->pending_intr,obj);
	...
	enable_irq(obj->irq);
        ...
        while(-1 == ltr553_get_ps_value(obj, obj->ps))
        {
            ...
            msleep(50);
            APS_ERR("ltr553 read ps data delay\n");;
        }
	...
	APS_LOG("ltr553_eint_work ps_thd_val_high: %d,ps_thd_val: %d\n", atomic_read(&obj->ps_thd_val_high),obj->ps_thd_val);
...
*/
	...
	{
		of_property_read_u32_array( ltr553_obj->irq_node, "debounce", ints, ARRAY_SIZE(ints));
		...
		if(request_irq( ltr553_obj->irq,  ltr553_eint_handler, IRQF_TRIGGER_NONE, "ALS-eint", NULL)) {
...
static ssize_t ltr553_show_als(struct device_driver *ddri, char *buf)
{
    ...
    if(res = ltr553_read_data_als(ltr553_obj->client, &ltr553_obj->als))
    {
...
#endif
    while(-1 == ltr553_get_als_value(ltr553_obj,ltr553_obj->als))
    {
        ltr553_read_data_als(ltr553_obj->client,&ltr553_obj->als);
        msleep(50);
    }
    dat = ltr553_get_als_value(ltr553_obj,ltr553_obj->als);
...
static ssize_t ltr553_show_ps(struct device_driver *ddri, char *buf)
{
    ...
    if(res = ltr553_read_data_ps(ltr553_obj->client, &ltr553_obj->ps))
    {
        return snprintf(buf, PAGE_SIZE, "ERROR: %d\n", res);
    }
    else
    {
        ...
        dat = ltr553_get_ps_value(ltr553_obj, ltr553_obj->ps);
        //return snprintf(buf, PAGE_SIZE, "0x%04X\n", dat);
        return snprintf(buf, PAGE_SIZE, "0x%04X\n", ltr553_obj->ps);
    }
}
...
static ssize_t ltr553_store_reg(struct device_driver *ddri,char *buf,ssize_t count)
{
    ...
    else if(2 != read_int_from_buf(ltr553_obj,buf,count,data,2))
    {
        APS_ERR("invalid format:%s\n",buf);
        return 0;
    }

    hwmsen_write_byte(ltr553_obj->client,data[0],data[1]);
...
//	struct alsps_hw *hw = get_cust_alsps_hw();
    ...
    for (i = 0; i < count; i++)
    {
        ...
        ret=ltr553_read_data_ps(ltr553_obj->client,&data);
        if (ret < 0)
        {
            i--;
            continue;
...
APS_ERR("ltr553_dynamic_calibrate High:%d,Low:%d\n",hw->ps_threshold_high,hw->ps_threshold_low);	
    hwmsen_write_byte(ltr553_obj->client, 0x90, (u8)(hw->ps_threshold_high) & 0XFF);
    hwmsen_write_byte(ltr553_obj->client, 0x91, (u8)((hw->ps_threshold_high)>>8) & 0X07);
    hwmsen_write_byte(ltr553_obj->client, 0x92, (u8)(hw->ps_threshold_low) & 0xff);
    hwmsen_write_byte(ltr553_obj->client, 0x93, (u8)((hw->ps_threshold_low)>>8) & 0x07);
...
static DRIVER_ATTR(enable_als,   0664, ltr553_show_enable_als, ltr553_store_enable_als);
...
/*----------------------------------------------------------------------------*/
static int  ltr553_get_ps_value_gesture(struct ltr553_priv *obj, int ps)
{
    int val= -1;
    int invalid = 0;

    APS_ERR("ltr553_get_ps_value###:ps=%x,Hight=%x,Low=%x\n", ps,(hw->ps_threshold_high),(hw->ps_threshold_low));
...
//    struct alsps_hw *hw = get_cust_alsps_hw();
    ...
    APS_LOG("ps_cali_set_threshold:g_ps_base_value=%x, hw->ps_threshold_high=%x,hw->ps_threshold_low=%x \n",
    ...
    if( value_high > 0x7f0)  //2032,ps max value is 2047
    {
        ...
        APS_LOG("ps_cali_set_threshold: set value_high=0x32,value_low=0x23, please check the phone \n");
    }

    atomic_set(&ltr553_obj->ps_thd_val_high, value_high);
    atomic_set(&ltr553_obj->ps_thd_val_low,  value_low);
...
static void ps_cali_start(void)
{
    ...
    */
    ...
    for(vl_index = 0; vl_index < 4; vl_index++)
    {
	 ...
	 if(err = ltr553_read_data_ps(ltr553_obj->client, &ltr553_obj->ps))
	  {
			APS_ERR("read data error\n");
	  }
	 
        vl_read_ps = ltr553_obj->ps;
    ...
    APS_LOG("ps_cali_start:g_ps_base_value=%x \n",g_ps_base_value);
...
#endif
#if defined(CONFIG_CUSTOM_KERNEL_ALSPS_SUB)
int OrientationGsensorState;
extern int AudioChannel;
extern int GsensorState;
extern struct ltr553_sub_priv;
extern struct ltr553_sub_priv *ltr553_sub_obj;
extern int ltr553_sub_enable_ps(struct i2c_client *client, bool enable);
extern int ltr553_sub_read_data_ps(struct i2c_client *client, int *data);
extern int ltr553_sub_get_ps_value(struct ltr553_sub_priv *obj, int ps);
void SwitchPsForVoipCall(void)
{
    int err = 0;
	struct ltr553_priv *obj = NULL;
	struct ltr553_priv *sub_obj = NULL;
	printk("[xucs]SwitchPsForVoipCall start\n");
    printk("[xucs]AudioChannel = %d\n",AudioChannel);
	printk("[xucs]OrientationGsensorState = %d\n",OrientationGsensorState);
	printk("[xucs]GsensorState is %d\n",GsensorState);
	obj = (struct ltr553_priv *)ltr553_obj;
	sub_obj = (struct ltr553_priv *)ltr553_sub_obj;
	if(AudioChannel)
	{
	    printk("[xucs]Now is a voip call\n");
		if(GsensorState != OrientationGsensorState)
		{
		    printk("[xucs]Switch orientation\n");
			OrientationGsensorState = GsensorState;
			if(GsensorState == 1)
			{
                if(err = ltr553_sub_enable_ps(sub_obj->client, false))
                {
                    APS_ERR("disable ps fail: %d\n", err);
                    return -1;
                }
                clear_bit(CMC_BIT_PS, &sub_obj->enable);	
                if(err = ltr553_enable_ps(obj->client, true))
                {
                    APS_ERR("enable ps fail: %d\n", err);
                    return -1;
			    ...
			    printk("[xucs]Now is positive screen mode\n");
				
			} 
			else if(GsensorState == 2)
			{
                if(err = ltr553_enable_ps(obj->client, false))
                {
                    APS_ERR("disable ps fail: %d\n", err);
                ...
                clear_bit(CMC_BIT_PS, &obj->enable);
                if(err = ltr553_sub_enable_ps(sub_obj->client, true))
                {
                    APS_ERR("enable ps fail: %d\n", err);
                    return -1;
                }
                set_bit(CMC_BIT_PS, &sub_obj->enable);

			    printk("[xucs]Now is negtive screen mode\n");
			}
		}
	} else {
	    OrientationGsensorState = 0;
	}
	printk("[xucs]SwitchPsForVoipCall end\n\n\n");
}
#endif
        ...
        case SENSOR_ENABLE:
            ...
            else
            {
                ...
                if(value)
                {
                    ...
                    #if defined(CONFIG_CUSTOM_KERNEL_ALSPS_SUB)
			OrientationGsensorState = GsensorState;
					if((GsensorState == 1) || (GsensorState == 0)) {
                        if(err = ltr553_sub_enable_ps(sub_obj->client, false))
                        {
                            APS_ERR("disable ps fail: %d\n", err);
                            return -1;
        ...
        case SENSOR_GET_DATA:
            ...
            else
            {
				...
				if((GsensorState == 1) || (GsensorState == 0)) {
                    ...
                    sensor_data = (struct hwm_sensor_data *)buff_out;
                    if (err = ltr553_sub_read_data_ps(sub_obj->client, &sub_obj->ps))
                    {
                        err = -1;
                        break;
                    }
                    ...
                    {
                        while(-1 == ltr553_sub_get_ps_value(sub_obj, sub_obj->ps))
                        {
                            ltr553_sub_read_data_ps(sub_obj->client, &sub_obj->ps);
                            msleep(50);
                        }
                        sensor_data->values[0] = ltr553_sub_get_ps_value(sub_obj, sub_obj->ps);
                        sensor_data->value_divide = 1;
                        sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
                        APS_ERR("fwq get ps raw_data = %d, sensor_data =%d\n",sub_obj->ps, sensor_data->values[0]);
                    }
				}
				#else
                sensor_data = (struct hwm_sensor_data *)buff_out;
    ...
    switch (command)
    {
        ...
        case SENSOR_GET_DATA:
            ...
            else
            {
                ...
                {
                    //BEGIN<DHACN-833><20161031><Sometimes it reports 0 when out  of sleep mode>wangyanhui
                    static  int  count_flag = 0;					
			while((ltr553_obj->als_enable == 0)&&(count_flag < 12))
			{
                        msleep(100);
			   count_flag++ ;	
			   APS_ERR(" count_flag = %d \n " ,count_flag);
			}
			count_flag = 0;
			//END<DHACN-833><20161031><Sometimes it reports 0 when out  of sleep mode>wangyanhui			
                    ...
                    sensor_data->value_divide = 1;
                    sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
                    APS_ERR("ltr553_als_operate get obj->als = %d, sensor_data =%d\n",obj->als, sensor_data->values[0]);
                }
            }
            break;
        default:
...
static int ltr553_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef GN_MTK_BSP_ALSPS_INTERRUPT_MODE
    INIT_DELAYED_WORK(&obj->eint_work, ltr553_eint_work);
#endif
    obj->client = client;
    APS_LOG("addr = %x\n",obj->client->addr);
    ...
    {
        APS_ERR("ltr553 i2c transfer error, part_id:%d\n", part_id);
    ...
    if(err = ltr553_create_attr(&ltr553_init_info.platform_diver_addr->driver))//(&ltr553_alsps_driver.driver))
    {
    ...
    strcpy(gn_dev_info_light.name, ltr553_DEV_NAME);
    gn_set_device_info(gn_dev_info_light);

    gn_dev_info_proximity.gn_dev_type = GN_DEVICE_TYPE_PROXIMITY;
    strcpy(gn_dev_info_proximity.name, ltr553_DEV_NAME);
...
static int ltr553_i2c_remove(struct i2c_client *client)
{
    ...
    if(err = ltr553_delete_attr(&ltr553_init_info.platform_diver_addr->driver))//(&ltr553_i2c_driver.driver))
    {
...
int ltr553_get_ps_status(void)
{
    ...
    do 
    {     
        err = hwmsen_read_byte_sr(ltr553_obj->client,APS_RO_ALS_PS_STATUS,&buffer);
		printk("buffer>>>%d\n",buffer);
    ...
    // Get PS status
    ltr553_read_data_ps(obj->client, &obj->ps);
    ps_status = ltr553_get_ps_value_gesture(obj, obj->ps);	
...
EXPORT_SYMBOL(ltr553_get_ps_status);
