...
static int bmg_read_raw_data(struct i2c_client *client, s16 data[BMG_AXES_NUM])
{
	...
	if (priv->sensor_type == BMI160_GYRO_TYPE) {
		...
		GYRO_LOG("[BMI120_GYRO]%s :[%08X %08X %08X] => [%5d %5d %5d]\n",__func__,
			data[BMG_AXIS_X],
			data[BMG_AXIS_Y],
			data[BMG_AXIS_Z],
			data[BMG_AXIS_X],
		...
		char *buf, int bufsize)
{
	...
	if (err) {
		...
		/* compensate data */
		GYRO_LOG("[BMI120_GYRO]%s :raw databuf[%08X %08X %08X] => [%5d %5d %5d]\n",__func__,
		databuf[BMG_AXIS_X],
		databuf[BMG_AXIS_Y],
		databuf[BMG_AXIS_Z],
		databuf[BMG_AXIS_X],
		databuf[BMG_AXIS_Y],
		databuf[BMG_AXIS_Z]);
		GYRO_LOG("[BMI120_GYRO]%s :obj->cali_sw[%08X %08X %08X] => [%5d %5d %5d]\n",__func__,
		obj->cali_sw[BMG_AXIS_X],
		obj->cali_sw[BMG_AXIS_Y],
		obj->cali_sw[BMG_AXIS_Z],
		obj->cali_sw[BMG_AXIS_X],
		...
		/* remap coordinate */
		GYRO_LOG("[BMI120_GYRO]%s :add cali databuf[%08X %08X %08X] => [%5d %5d %5d]\n",__func__,
		databuf[BMG_AXIS_X],
		databuf[BMG_AXIS_Y],
		databuf[BMG_AXIS_Z],
		databuf[BMG_AXIS_X],
		databuf[BMG_AXIS_Y],
		databuf[BMG_AXIS_Z]);
		GYRO_LOG("[BMI120_GYRO]%s :cvt.map[%d %d %d]\n",__func__,
		obj->cvt.map[BMG_AXIS_X],
		obj->cvt.map[BMG_AXIS_Y],
		obj->cvt.map[BMG_AXIS_Z]);
		GYRO_LOG("[BMI120_GYRO]%s :cvt.sign[%d %d %d]\n",__func__,
		obj->cvt.sign[BMG_AXIS_X],
		obj->cvt.sign[BMG_AXIS_Y],
		obj->cvt.sign[BMG_AXIS_Z]);
		...
		GYRO_LOG("[BMI120_GYRO]%s before mapping :\n",__func__);
		GYRO_LOG("[BMI120_GYRO]%s :gyro[%08X %08X %08X] => [%5d %5d %5d]\n",__func__,
		gyro[obj->cvt.map[BMG_AXIS_X]],
		gyro[obj->cvt.map[BMG_AXIS_Y]],
		gyro[obj->cvt.map[BMG_AXIS_Z]],
		gyro[obj->cvt.map[BMG_AXIS_X]],
		gyro[obj->cvt.map[BMG_AXIS_Y]],
		gyro[obj->cvt.map[BMG_AXIS_Z]]);
		GYRO_LOG("[BMI120_GYRO]%s after mapping :\n",__func__);
		GYRO_LOG("[BMI120_GYRO]%s :gyro[%08X %08X %08X] => [%5d %5d %5d]\n",__func__,
		gyro[BMG_AXIS_X],
		gyro[BMG_AXIS_Y],
		gyro[BMG_AXIS_Z],
		...
		gyro[BMG_AXIS_Y],
		gyro[BMG_AXIS_Z]);
		gyro[BMG_AXIS_X] = gyro[BMG_AXIS_X] * BMI160_FS_MAX_LSB / BMI160_FS_2000_LSB;
		gyro[BMG_AXIS_Y] = gyro[BMG_AXIS_Y] * BMI160_FS_MAX_LSB / BMI160_FS_2000_LSB;
		gyro[BMG_AXIS_Z] = gyro[BMG_AXIS_Z] * BMI160_FS_MAX_LSB / BMI160_FS_2000_LSB;
		GYRO_LOG("[BMI120_GYRO]%s convert over:\n",__func__);
		GYRO_LOG("[BMI120_GYRO]%s :gyro[%08X %08X %08X] => [%5d %5d %5d]\n",__func__,
		gyro[BMG_AXIS_X],
		gyro[BMG_AXIS_Y],
		gyro[BMG_AXIS_Z],
		...
		unsigned long arg)
{
	...
	case GYROSCOPE_IOCTL_READ_SENSORDATA:
		GYRO_LOG("[BMI120]%s:GYROSCOPE_IOCTL_READ_SENSORDATA\n",__func__);
		data = (void __user *) arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
	...
	case GYROSCOPE_IOCTL_SET_CALI:
		...
		GYRO_LOG("[BMI120]%s:GYROSCOPE_IOCTL_SET_CALI\n",__func__);
		data = (void __user *)arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
		...
		if (atomic_read(&obj->suspend)) {
			...
			cali[BMG_AXIS_X] = sensor_data.x * BMI160_FS_2000_LSB / BMI160_FS_MAX_LSB;
			cali[BMG_AXIS_Y] = sensor_data.y * BMI160_FS_2000_LSB / BMI160_FS_MAX_LSB;
			cali[BMG_AXIS_Z] = sensor_data.z * BMI160_FS_2000_LSB / BMI160_FS_MAX_LSB;
			err = bmg_write_calibration(client, cali);
		}
		break;
	case GYROSCOPE_IOCTL_CLR_CALI:
		GYRO_LOG("[BMI120]%s:GYROSCOPE_IOCTL_CLR_CALI\n",__func__);
		err = bmg_reset_calibration(client);
		break;
	case GYROSCOPE_IOCTL_GET_CALI:
		GYRO_LOG("[BMI120]%s:GYROSCOPE_IOCTL_GET_CALI\n",__func__);
		data = (void __user *)arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
		...
		sensor_data.x = cali[BMG_AXIS_X] * BMI160_FS_MAX_LSB / BMI160_FS_2000_LSB ;
		sensor_data.y = cali[BMG_AXIS_Y] * BMI160_FS_MAX_LSB / BMI160_FS_2000_LSB ;
		sensor_data.z = cali[BMG_AXIS_Z] * BMI160_FS_MAX_LSB / BMI160_FS_2000_LSB ;
		if (copy_to_user(
